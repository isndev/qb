#!/usr/bin/env python3
"""Lint: forbid spawn(<immediately-invoked coroutine lambda>).
...
for *.cpp / *.h / *.hpp and exits non-zero listing every offending site.
    ...
    for root in roots:
        ...
        for f in sorted(set(files)):
            if os.sep + "build" + os.sep in f or os.sep + "modules" + os.sep in f:
                continue
            for line in find_sites(f):
                failed = True
                print(
                    f"{f}:{line}: error: spawn(<immediately-invoked lambda>) — "
                    f"dangling closure (UB). Drop the trailing '()' so the "
                    f"spawn(Callable) overload owns the closure."
                )
    if failed:
        return 1
    print("OK: no spawn(<immediately-invoked lambda>) sites found.")
    return 0
