 ...
 */
 ...
 quota_for(std::size_t const producer_index,
 ...
 run_router_mailbox(std::size_t const nb_producers,
                    std::uint64_t const total,
                    std::size_t const dequeue_batch) {
 ...
 BM_MpscRouterMailbox_FanIn(benchmark::State &state) {
     ...
     const double bytes_reserved =
         static_cast<double>(nb_producers) *
         static_cast<double>(MailboxCap) *
         static_cast<double>(sizeof(EventBucket));
 
     const double bytes_reserved_per_producer =
         static_cast<double>(MailboxCap) * static_cast<double>(sizeof(EventBucket));
     ...
     for (auto _ : state) {
         ...
         state.counters["enqueue_failures"] =
             benchmark::Counter(static_cast<double>(result.enqueue_failures),
