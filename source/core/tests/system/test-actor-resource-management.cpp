...
struct AllocateResourceEvent : public qb::Event {
    int size;
    explicit AllocateResourceEvent(int s) : size(s) {}
};

struct ReleaseResourceEvent : public qb::Event {
    int resource_id;
    explicit ReleaseResourceEvent(int id) : resource_id(id) {}
};

struct ResourceStatusEvent : public qb::Event {
    qb::ActorId reply_to;
    explicit ResourceStatusEvent(qb::ActorId id) : reply_to(id) {}
};
...
struct ResourceReportEvent : public qb::Event {
    ...
    ResourceReportEvent(int count, size_t memory) 
        : allocated_count(count), memory_usage(memory) {}
};
...
public:
    explicit ManagedResource(int id, size_t size) : _id(id), _data(size) {
        // Track allocation
...
public:
    ResourceActor() : _shutdown_pending(false) {}
    ...
    void on(const AllocateResourceEvent& event) {
    ...
    void on(const ReleaseResourceEvent& event) {
    ...
    void on(const ResourceStatusEvent& event) {
        int count = 0;
        size_t memory = 0;
    ...
    void on(const GracefulShutdownEvent&) {
    ...
    void on(const ForceShutdownEvent&) {
        kill();
    }
...
public:
    explicit ResourceCoordinatorActor(int num_allocations = 100)
        : _num_allocations(num_allocations), _phase(0) {}
    ...
    void on(const ResourceReportEvent& event) {
