...
struct StateUpdateEvent : public qb::Event {
    ...
    StateUpdateEvent(const std::string& k, int v) : key(k), value(v) {}
};
...
struct StateQueryEvent : public qb::Event {
    ...
    StateQueryEvent(const std::string& k, qb::ActorId id) : key(k), reply_to(id) {}
};
...
struct StateResponseEvent : public qb::Event {
    ...
    StateResponseEvent(const std::string& k, int v, bool f) : key(k), value(v), found(f) {}
};
...
public:
    ...
    void on(const StateUpdateEvent& event) {
        if (_failed) return; // Simulate failure by ignoring updates
        
        _state[event.key] = event.value;
    }
    
    // Handle state queries
    void on(const StateQueryEvent& event) {
        if (_failed) return; // Simulate failure by ignoring queries
    ...
    void on(const SimulateFailureEvent&) {
        _failed = true;
    }
    
    // Restore state from "persistent storage" (in this test, we just reset the failed flag)
    void on(const RestoreStateEvent&) {
        _failed = false;
        g_state_recovered = true;
    }
    
    // Create a checkpoint of the state (in a real system, this would write to disk/database)
    void on(const CheckpointEvent&) {
        if (_failed) return;
    ...
    void on(const VerifyStateEvent&) {
        if (_failed) return;
    ...
    // Complete test
    void on(const TestCompleteEvent&) {
        kill();
    }
};
...
public:
    ...
    void scheduleTestSequence() {
        ...
        qb::io::async::callback([this]() {
            to(_stateful_actor_id).push<SimulateFailureEvent>();
        }, 0.15);
        ...
        qb::io::async::callback([this]() {
            to(_stateful_actor_id).push<RestoreStateEvent>();
        }, 0.25);
        
        // Verify state after 300ms
        qb::io::async::callback([this]() {
            to(_stateful_actor_id).push<VerifyStateEvent>();
        }, 0.3);
    ...
    void on(const StateResponseEvent& event) {
