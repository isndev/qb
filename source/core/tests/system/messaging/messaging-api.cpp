...
protected:
    qb::Main main;
    void SetUp() override { g_msg_delivered.store(0, std::memory_order_relaxed); }
...
protected:
    ...
    std::uint32_t expected_deliveries() const override { return MAX_ACTORS * MAX_EVENTS; }
};
...
protected:
    ...
    std::uint32_t expected_deliveries() const override { return max_core * MAX_ACTORS * MAX_EVENTS; }
};
...
TYPED_TEST(ActorEventMono, SendEvents) { this->run_and_verify(); }
TYPED_TEST(ActorEventBroadcastMono, SendEvents) { this->run_and_verify(); }
...
struct BroadcastValueEvent : public qb::Event {
    int value;
    explicit BroadcastValueEvent(int v) : value(v) {}
};
...
public:
    ...
    void on(const ProbeEvent &) { g_unreg_received.fetch_add(1, std::memory_order_relaxed); }
    void on(const ControlTick &) {
...
public:
    explicit UnregisterDriverActor(qb::ActorId probe) : _probe(probe) {}
    qb::io::async::task<bool> onInit() override {
