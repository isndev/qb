...
struct ReproReceiver : qb::Actor {
    bool onInit() override { registerEvent<qb::KillEvent>(*this); return true; }
};

struct ReproSender : qb::Actor {
    qb::ActorId target;
    explicit ReproSender(qb::ActorId t) : target(t) {}
...
TEST(AuditRepro, UnregisteredEventTypeMustNotKillCore) {
    ...
    EXPECT_FALSE(engine.hasError())
        << "destination core died after receiving an unregistered event type";
}
...
struct CbVictim : qb::Actor, qb::ICallback {
    bool onInit() override {
        registerEvent<qb::KillEvent>(*this);
        registerCallback(*this);
        return true;
    }
    void onCallback() override { g_victim_ticks.fetch_add(1, std::memory_order_relaxed); }
    void on(qb::KillEvent const &) { kill(); }
};

struct CbKiller : qb::Actor, qb::ICallback {
...
TEST(AuditRepro, KilledActorGetsNoFurtherCallbackInSamePass) {
    ...
    EXPECT_EQ(g_victim_ticks.load(), 0)
        << "victim ticked after being killed in the same callback pass";
}
