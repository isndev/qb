...
struct ErrorInducingEvent : public qb::Event {
    ...
    explicit ErrorInducingEvent(ErrorType type) : error_type(type) {}
};

struct MonitorEvent : public qb::Event {
    qb::ActorId target_id;
    explicit MonitorEvent(qb::ActorId id) : target_id(id) {}
};

struct StatusEvent : public qb::Event {
    bool is_alive;
    explicit StatusEvent(bool alive) : is_alive(alive) {}
};
...
public:
    ...
    void on(const ErrorInducingEvent& event) {
    ...
    void on(const MonitorEvent& event) {
...
public:
    explicit MonitorActor(int num_actors) 
        : _num_actors_to_monitor(num_actors), _num_actors_checked(0) {}
    ...
    bool onInit() override {
        ...
        // Use async callback to broadcast KillEvent to terminate all actors after timeout
    ...
    void addActorToMonitor(qb::ActorId actor_id) {
    ...
    void on(const StatusEvent& event) {
...
public:
    CoordinatorActor(int num_actors, ErrorInducingEvent::ErrorType error_type, bool should_recover) 
        : _error_type(error_type), _should_actors_recover(should_recover), _num_actors(num_actors) {}
...
TEST(ErrorHandling, ShouldRecoverFromErrors) {
    ...
    main.addActor<CoordinatorActor>(
        0,
        num_actors, 
