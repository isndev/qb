...
struct IncrementEvent : public qb::Event {
    ...
    IncrementEvent(int id, int inc) : counter_id(id), increment_by(inc) {}
};

struct QueryCountersEvent : public qb::Event {
    qb::ActorId reply_to;
    
    explicit QueryCountersEvent(qb::ActorId id) : reply_to(id) {}
};

struct CountersResponseEvent : public qb::Event {
    std::vector<int> counter_values;
    
    explicit CountersResponseEvent(std::vector<int> values) : counter_values(std::move(values)) {}
};

struct WorkerCompleteEvent : public qb::Event {
    int worker_id;
    
    explicit WorkerCompleteEvent(int id) : worker_id(id) {}
};
...
public:
    ...
    void on(const QueryCountersEvent& event) {
...
public:
    ...
    bool onInit() override {
        ...
        // Create worker actors - chaque worker fait exactement NUM_OPERATIONS / NUM_WORKERS opérations
    ...
    void on(const WorkerCompleteEvent& event) {
        _active_workers--;
    ...
    void finalizeTest() {
        if (_test_completed) return; // Prevent double completion
        ...
        qb::io::async::callback([this]() {
            ...
            // Give counter actor time to process the complete event before terminating
            qb::io::async::callback([this]() {
                kill();
            }, 0.1);
        }, 0.2);
    ...
    void on(const CountersResponseEvent& event) {
...
TEST(ConcurrencySafety, ShouldHandleConcurrentOperationsSafely) {
    ...
    // Le nombre exact d'opérations peut varier, mais devrait être proche de NUM_OPERATIONS
