...
namespace qb {
    ...
    bool CoreInitializer::ActorBuilder::valid() const noexcept { return _valid; }
    CoreInitializer::ActorBuilder::operator bool() const noexcept { return valid(); }
    ...
    bool SharedCoreCommunication::send(Event const &event) const noexcept {
        ...
        return static_cast<bool>(_mail_boxes[_core_set.resolve(event.dest._index)]->enqueue(source_index,
                                                                          reinterpret_cast<const EventBucket *>(&event),
                                                                          event.bucket_size));
    }

    SharedCoreCommunication::MPSCBuffer &SharedCoreCommunication::getMailBox(CoreId const id) const noexcept {
        ...
        if (!_core_initializers.empty()) {
            ...
            for (auto &it : _core_initializers) {
                if (!async && i == (_core_initializers.size() - 1))
                    ...
                    _cores[i] = std::thread(start_thread, CoreSpawnerParameter{it.first, it.second, *_shared_com});
                ++i;
            }
