...
void VirtualCore::__init__actors__() const {
    // Init StaticActors
    if (std::any_of(_actors.begin(), _actors.end(),
                    [](auto &it) { return !it.second->onInit(); })) {
...
void VirtualCore::__workflow__() {
    ...
    if (!Main::is_running) {
        LOG_INFO("" << *this << " Stopped by user " << static_cast<uint32_t>(_actors.size())
                    << " actor(s) left");
    } else {
...
CoreId VirtualCore::getIndex() const noexcept {
    return _index;
}

uint64_t VirtualCore::time() const noexcept {
