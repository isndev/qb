...
crypto::get_evp_md(DigestAlgorithm algorithm) {
    switch (algorithm) {
        ...
        case DigestAlgorithm::SHA224: return EVP_sha224();
        ...
        case DigestAlgorithm::BLAKE2S256: return EVP_blake2s256();
    ...
    const std::vector<unsigned char>& aad)
{
    ...
    if (!ctx) {
        throw std::runtime_error("Failed to create cipher context: " + get_openssl_error());
    }
    ...
    try {
        ...
        if (is_aead && !aad.empty()) {
            if (EVP_EncryptUpdate(ctx, nullptr, &out_len, aad.data(), aad.size()) != 1) {
                throw std::runtime_error("Failed to process AAD: " + get_openssl_error());
            }
        }
    ...
    const std::vector<unsigned char>& aad)
{
    ...
    std::vector<unsigned char> plaintext(actual_ciphertext.size() + EVP_MAX_BLOCK_LENGTH);
    int out_len = 0;
    int final_len = 0;
    ...
    try {
        ...
        // Decrypt ciphertext
        if (EVP_DecryptUpdate(ctx, plaintext.data(), &out_len, actual_ciphertext.data(), actual_ciphertext.size()) != 1) {
        ...
        if (ret != 1) {
            ...
            if (is_aead) {
                return std::vector<unsigned char>(); // Return empty vector on auth failure
            } else {
    ...
    DigestAlgorithm algorithm)
{
    ...
    if (!ctx) {
        throw std::runtime_error("Failed to create digest context: " + get_openssl_error());
    }
    ...
    DigestAlgorithm algorithm)
{
    ...
    try {
        ...
        if (!pkey) {
            ...
            throw std::runtime_error("Failed to create HMAC key: " + get_openssl_error());
        }
        ...
        if (EVP_DigestSignInit(mdctx, nullptr, md, nullptr, pkey) != 1) {
            EVP_PKEY_free(pkey);
            EVP_MD_CTX_free(mdctx);
            throw std::runtime_error("Failed to initialize HMAC: " + get_openssl_error());
        }
        ...
        if (EVP_DigestSignFinal(mdctx, nullptr, &hmac_len) != 1) {
            EVP_PKEY_free(pkey);
            EVP_MD_CTX_free(mdctx);
            throw std::runtime_error("Failed to determine HMAC size: " + get_openssl_error());
        }
