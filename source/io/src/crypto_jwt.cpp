...
std::string
base64url_to_base64(const std::string &base64url) {
    ...
    switch (base64.length() % 4) {
        ...
        case 2:
            base64.append("==");
            break; // Add two padding characters
        case 3:
            base64.append("=");
            break; // Add one padding character
        default:
...
std::string
base64_to_base64url(const std::string &base64) {
...
jwt::sign_data(const std::string &data, const CreateOptions &options) {
    // Convert string key to bytes for HMAC algorithms
    std::vector<unsigned char> key_bytes;
    if (options.algorithm == Algorithm::HS256 || options.algorithm == Algorithm::HS384 || options.algorithm == Algorithm::HS512) {
...
bool
jwt::verify_signature(const std::string &data, const std::vector<unsigned char> &signature, const VerifyOptions &options) {
    // Convert string key to bytes for HMAC algorithms
    std::vector<unsigned char> key_bytes;
    if (options.algorithm == Algorithm::HS256 || options.algorithm == Algorithm::HS384 || options.algorithm == Algorithm::HS512) {
        key_bytes.assign(options.key.begin(), options.key.end());
    ...
    switch (options.algorithm) {
        ...
        case Algorithm::RS384:
        case Algorithm::RS512:
            return crypto::rsa_verify(data_bytes, signature, options.key, get_digest_algorithm(options.algorithm));
...
jwt::get_digest_algorithm(Algorithm algorithm) {
    switch (algorithm) {
        case Algorithm::HS256:
        case Algorithm::RS256:
        case Algorithm::ES256:
...
std::string
jwt::algorithm_to_string(Algorithm algorithm) {
    switch (algorithm) {
        case Algorithm::HS256:
            return "HS256";
        case Algorithm::HS384:
...
std::optional<jwt::Algorithm>
jwt::algorithm_from_string(const std::string &algorithm_str) {
    if (algorithm_str == "HS256")
        return Algorithm::HS256;
    if (algorithm_str == "HS384")
        return Algorithm::HS384;
    if (algorithm_str == "HS512")
        return Algorithm::HS512;
    if (algorithm_str == "RS256")
        return Algorithm::RS256;
    if (algorithm_str == "RS384")
        return Algorithm::RS384;
    if (algorithm_str == "RS512")
        return Algorithm::RS512;
    if (algorithm_str == "ES256")
        return Algorithm::ES256;
    if (algorithm_str == "ES384")
        return Algorithm::ES384;
    if (algorithm_str == "ES512")
        return Algorithm::ES512;
    if (algorithm_str == "EdDSA")
...
std::string
jwt::create(const std::map<std::string, std::string> &payload, const CreateOptions &options) {
    // Create header
    json header_json;
    ...
    std::string header_b64 = base64_to_base64url(crypto::base64::encode(header_str));

    std::string payload_b64 = base64_to_base64url(crypto::base64::encode(payload_str));
    ...
    std::string signature_b64 = base64_to_base64url(crypto::base64::encode(std::string(signature.begin(), signature.end())));
...
std::string
jwt::create_token(const std::map<std::string, std::string> &payload, const std::string &issuer, const std::string &subject,
                  const std::string &audience, std::chrono::seconds expires_in, std::chrono::seconds not_before, const std::string &jti,
...
jwt::verify(const std::string &token, const VerifyOptions &options) {
    try {
        ...
        if (options.verify_issuer && options.issuer.has_value()) {
            if (!payload_json.contains("iss") || payload_json["iss"].get<std::string>() != options.issuer.value()) {
        ...
        if (options.verify_subject && options.subject.has_value()) {
            if (!payload_json.contains("sub") || payload_json["sub"].get<std::string>() != options.subject.value()) {
        ...
        if (options.verify_jti && options.jti.has_value()) {
            if (!payload_json.contains("jti") || payload_json["jti"].get<std::string>() != options.jti.value()) {
        ...
        for (const auto &[claim_name, claim_value] : options.required_claims) {
            if (!payload_json.contains(claim_name) || payload_json[claim_name].get<std::string>() != claim_value) {
                return ValidationResult(ValidationError::CLAIM_MISMATCH);
            }
        }
        ...
        for (const auto &[key, value] : payload_json.items()) {
            result.payload[key] = value.is_string() ? value.get<std::string>() : value.dump();
        }
