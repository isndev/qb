...
#ifdef _MSC_VER
#    pragma warning(disable : 4127) // "conditional expression is constant" generated by
                                    // the FD_SET macro
#endif
...
ip
socket::getRemoteAddress() const noexcept {
    if (is_open()) {
...
unsigned short
socket::getRemotePort() const noexcept {
    if (is_open()) {
        // Retrieve informations about the remote end of the socket
        sockaddr_in address;
        AddrLength size = sizeof(address);
...
socket::connect(const ip &remoteAddress, unsigned short const remotePort,
    ...
    if (!timeout) {
        ...
        // Connect the socket
        if (::connect(_handle, reinterpret_cast<sockaddr *>(&address),
                      sizeof(address)) == -1)
        ...
        // Otherwise, wait until something happens to our socket (success, timeout or
        // error)
        if (status == SocketStatus::NotReady) {
            ...
            // Wait for something to write on our socket (which means that the connection
            // request has returned)
            if (select(static_cast<int>(_handle + 1), NULL, &selector, NULL, &time) >
                0) {
...
int
socket::read(void *data, std::size_t const size) const noexcept {
    const auto ret =
        ::recv(_handle, static_cast<char *>(data), static_cast<int>(size), flags);
    return ret ? ret : -1;
}

int
socket::write(const void *data, std::size_t const size) const noexcept {
    return ::send(_handle, static_cast<const char *>(data), static_cast<int>(size),
                  flags);
}
