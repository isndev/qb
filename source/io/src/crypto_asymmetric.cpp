 ...
 * 
 * This file provides implementations of modern asymmetric cryptographic operations such as:
...
static std::string get_openssl_asymmetric_error() {
    char err_buf[256];
...
static std::string key_to_pem(EVP_PKEY* pkey, bool is_private) {
    ...
    if (result != 1) {
        BIO_free(bio);
        throw std::runtime_error("Failed to write key to PEM: " + get_openssl_asymmetric_error());
    }
...
static EVP_PKEY* pem_to_key(const std::string& pem_str, bool is_private) {
...
static std::vector<unsigned char> get_raw_key_bytes(EVP_PKEY* pkey, bool is_private) {
    size_t key_len;
    if (EVP_PKEY_get_raw_private_key(pkey, NULL, &key_len) != 1 && 
        EVP_PKEY_get_raw_public_key(pkey, NULL, &key_len) != 1) {
        throw std::runtime_error("Failed to determine key length: " + get_openssl_asymmetric_error());
    }
    
    std::vector<unsigned char> key_bytes(key_len);
    if (is_private) {
        if (EVP_PKEY_get_raw_private_key(pkey, key_bytes.data(), &key_len) != 1) {
            throw std::runtime_error("Failed to extract private key bytes: " + get_openssl_asymmetric_error());
        }
    } else {
        if (EVP_PKEY_get_raw_public_key(pkey, key_bytes.data(), &key_len) != 1) {
            throw std::runtime_error("Failed to extract public key bytes: " + get_openssl_asymmetric_error());
        }
    }
...
crypto::generate_ed25519_keypair() {
    ...
    if (!ctx) {
        throw std::runtime_error("Failed to create Ed25519 context: " + get_openssl_asymmetric_error());
    }
    
    // Initialize key generation operation
    if (EVP_PKEY_keygen_init(ctx) != 1) {
        EVP_PKEY_CTX_free(ctx);
        throw std::runtime_error("Failed to initialize Ed25519 key generation: " + get_openssl_asymmetric_error());
    }
    ...
    if (EVP_PKEY_keygen(ctx, &pkey) != 1) {
        EVP_PKEY_CTX_free(ctx);
        throw std::runtime_error("Ed25519 key generation failed: " + get_openssl_asymmetric_error());
    }
...
crypto::ed25519_sign(
    ...
    if (!md_ctx) {
        EVP_PKEY_free(pkey);
        throw std::runtime_error("Failed to create signing context: " + get_openssl_asymmetric_error());
    }
    ...
    if (EVP_DigestSignInit(md_ctx, NULL, NULL, NULL, pkey) != 1) {
        ...
        EVP_PKEY_free(pkey);
        throw std::runtime_error("Failed to initialize signing operation: " + get_openssl_asymmetric_error());
    }
    ...
    if (EVP_DigestSign(md_ctx, NULL, &sig_len, data.data(), data.size()) != 1) {
        EVP_MD_CTX_free(md_ctx);
        EVP_PKEY_free(pkey);
        throw std::runtime_error("Failed to determine signature size: " + get_openssl_asymmetric_error());
    }
...
crypto::ed25519_verify(
    ...
    if (!md_ctx) {
        EVP_PKEY_free(pkey);
        throw std::runtime_error("Failed to create verification context: " + get_openssl_asymmetric_error());
    }
    ...
    if (EVP_DigestVerifyInit(md_ctx, NULL, NULL, NULL, pkey) != 1) {
        EVP_MD_CTX_free(md_ctx);
        EVP_PKEY_free(pkey);
        throw std::runtime_error("Failed to initialize verification operation: " + get_openssl_asymmetric_error());
    }
...
crypto::generate_x25519_keypair() {
    ...
    if (!ctx) {
        throw std::runtime_error("Failed to create X25519 context: " + get_openssl_asymmetric_error());
    }
    
    // Initialize key generation operation
    if (EVP_PKEY_keygen_init(ctx) != 1) {
        EVP_PKEY_CTX_free(ctx);
        throw std::runtime_error("Failed to initialize X25519 key generation: " + get_openssl_asymmetric_error());
    }
    ...
    if (EVP_PKEY_keygen(ctx, &pkey) != 1) {
        EVP_PKEY_CTX_free(ctx);
        throw std::runtime_error("X25519 key generation failed: " + get_openssl_asymmetric_error());
    }
...
crypto::x25519_key_exchange(
    ...
    if (!ctx) {
        ...
        throw std::runtime_error("Failed to create key exchange context: " + get_openssl_asymmetric_error());
    }
    ...
    if (EVP_PKEY_derive_init(ctx) != 1) {
        ...
        EVP_PKEY_free(priv_key);
        EVP_PKEY_free(pub_key);
        throw std::runtime_error("Failed to initialize key derivation: " + get_openssl_asymmetric_error());
    }
    ...
    if (EVP_PKEY_derive(ctx, NULL, &secret_len) != 1) {
        ...
        EVP_PKEY_free(priv_key);
        EVP_PKEY_free(pub_key);
        throw std::runtime_error("Failed to determine shared secret length: " + get_openssl_asymmetric_error());
    }
    ...
    if (EVP_PKEY_derive(ctx, shared_secret.data(), &secret_len) != 1) {
        ...
        EVP_PKEY_free(priv_key);
        EVP_PKEY_free(pub_key);
        throw std::runtime_error("Key derivation failed: " + get_openssl_asymmetric_error());
    }
...
crypto::x25519_key_exchange(
    ...
    EVP_PKEY* priv_key = EVP_PKEY_new_raw_private_key(EVP_PKEY_X25519, NULL, 
                                                     private_key_bytes.data(), 
                                                     private_key_bytes.size());
    if (!priv_key) {
    ...
    EVP_PKEY* pub_key = EVP_PKEY_new_raw_public_key(EVP_PKEY_X25519, NULL, 
...
crypto::ecies_encrypt(
    ...
    std::vector<unsigned char> shared_secret = x25519_key_exchange(
        ephemeral_priv_key, recipient_public_key);
    
    // Derive encryption key and IV using HKDF
    std::vector<unsigned char> key_material = hkdf(
        shared_secret,
    ...
    std::vector<unsigned char> encrypted_data = encrypt(
        data, symmetric_key, iv, sym_algorithm);
...
crypto::ecies_decrypt(
    ...
    std::vector<unsigned char> decrypted_data = decrypt(
        encrypted_data, symmetric_key, iv, sym_algorithm);
