/**
 * @file test_coroutine_benchmark.cpp
 * @brief Benchmark tests: Coroutines vs Callbacks
 *
 * Performance comparison between coroutine-based and callback-based
 * asynchronous operations. These tests help quantify the overhead
 * of coroutines compared to traditional callback patterns.
 *
...
TEST_F(CoroutineBenchmark, DISABLED_TimerLatency) {
    ...
    auto coro_ms = std::chrono::duration_cast<std::chrono::milliseconds>(coro_end - coro_start).count();
    ...
    struct CallbackTimer {
        ...
        CallbackTimer(std::atomic<int>& c, ev::loop_ref loop) : counter(c) {
    ...
    while (cb_completed < iterations && 
           std::chrono::steady_clock::now() - cb_wait_start < 200ms) {
...
TEST_F(CoroutineMemoryBenchmark, CoroutineFrameAllocation) {
    ...
    auto large_ms = std::chrono::duration_cast<std::chrono::milliseconds>(large_end - large_start).count();
...
TEST_F(CoroutineThroughput, MaximumSpawnRate) {
    ...
    auto spawn_us = std::chrono::duration_cast<std::chrono::microseconds>(spawn_end - spawn_start).count();
...
TEST_F(CoroutineLatency, TimerAccuracyDistribution) {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        auto coro_fn = [start = std::chrono::steady_clock::now(), actual_durations_ptr, duration_ms]() -> task<void> {
