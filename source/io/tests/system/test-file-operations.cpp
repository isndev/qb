...
TEST_F(FileSystemTest, FileToPipeAdvanced) {
    ...
    // On some platforms (like MacOS), the implementation may read the entire file at
    // once, so we can't reliably test multiple reads. Instead, test that reading works
    // correctly.
    int bytes_read = f2p.read();
    EXPECT_GT(bytes_read, 0);
...
TEST_F(FileSystemTest, VeryLargeFileTransfer) {
...

    {
        ...
        for (std::size_t i = 0; i < chunks_to_write && file.good(); ++i) {
