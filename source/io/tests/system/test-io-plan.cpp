...
TEST_F(IoPlanTest, T_UDP_WRITE_NO_OFFSET_PartialSendFreeFromQueue) {
    ...
    EXPECT_GT(written, 0) << "write() should have transmitted the datagram";
    ...
    EXPECT_EQ(sender.pendingWrite(), 0u) << "after a successful write() the output queue must be drained "
...
TEST_F(IoPlanTest, T_INPUT_DRAINED_VS_EOF_AliasAndDispatch) {
    static_assert(std::is_same_v<async::event::eof, async::event::input_drained>, "event::eof must be the input_drained alias");
    ...
    ASSERT_TRUE(server.transport().listen_v4(kPort) == 0) << "could not bind test server on " << kPort;
    server.start();

    // Connect a raw TCP client and send one framed message ("hello\n").
    qb::io::tcp::socket client;
    ASSERT_EQ(client.connect_v4("127.0.0.1", kPort), qb::io::SocketStatus::Done);
    ...
    ASSERT_GT(server.session_count(), 0u) << "server failed to register the test session";
    ...
    EXPECT_EQ(messages_seen.load(), 1) << "the server-side session should have parsed exactly one message";
    ...
    EXPECT_GE(drained_counter.load(), 1) << "input_drained must be dispatched once the buffer has been emptied "
                                            "(finding 2.9)";
...
TEST_F(IoPlanTest, T_REGISTER_SESSION_DOS_RejectsBeforeAllocation) {
    ...
    for (std::size_t i = 0; i < kBurst; ++i) {
        ...
        if (registered != nullptr) {
            ...
            EXPECT_EQ(handler.session_count(), kCap) << "rejected attempts must not perturb session_count";
        }
    }
...
TEST_F(IoPlanTest, T_SCOPED_CALLBACK_DestructionCancelsPendingTimer) {
    ...
    EXPECT_EQ(fired.load(), 0) << "destroying a ScopedTimeout before its deadline must cancel the callback";
}
...
TEST_F(IoPlanTest, T_ACCEPTOR_LISTEN_AutoStartsWatcher) {
    ...
    ASSERT_TRUE(server.transport().listen_v4(kPort) == 0) << "binding listener must succeed";
    server.start();
    ...
    EXPECT_EQ(connection_count.load(), 1) << "auto-started acceptor must accept the test client";
...
// regression in correctness on repeated broadcasts). Allocation amortisation is
// covered by `qb/source/io/tests/benchmark/bm-io-plan.cpp`.
