...
struct CompressionTestResult {
    ...
    to_string() const {
        std::stringstream ss;
        ss << "Level: " << level_name << ", Original: " << original_size << " bytes"
           << ", Compressed: " << compressed_size << " bytes"
           << ", Ratio: " << compression_ratio << ", Comp Time: " << compression_time_ms
           << " ms"
...
TEST(CompressionLevels, GzipLevels) {
    ...
    test_data +=
        qb::crypto::generate_random_string(250000, qb::crypto::range_alpha_numeric);
    ...
    for (const auto &level_info : compression_levels) {
        ...
        // Use make_gzip_compressor with custom level parameter and default values for
        // other parameters
        ...
        auto        compressed_size = compressor->compress(
            ...
            compressed_buffer.size(), qb::compression::is_last, input_processed, done);
        ...
        result.compression_ratio =
            static_cast<double>(test_data.size()) / compressed_size;

        // Buffer for decompressed data
        qb::allocator::pipe<char> decompressed_buffer;
        decompressed_buffer.allocate_back(test_data.size() +
                                          1000); // Extra space just in case
        ...
        auto decompressed_size = decompressor->decompress(
            ...
            decompressed_buffer.size(), qb::compression::is_last, input_processed, done);
...
TEST(CompressionLevels, DeflateLevels) {
    ...
    test_data += qb::crypto::generate_random_string(
        200000, qb::crypto::range_alpha_numeric_special);
...
TEST(CompressionLevels, DataTypeCompression) {
    ...
    auto compressor =
        qb::compression::builtin::make_gzip_compressor(6, // compressionLevel (balanced)
                                                       Z_DEFLATED,         // method
                                                       Z_DEFAULT_STRATEGY, // strategy
                                                       8                   // memLevel
        );
    ...
    std::vector<std::pair<std::string, std::string &>> test_data = {
        {"Text", text_data}, {"Binary", binary_data}, {"Pattern", pattern_data}};
