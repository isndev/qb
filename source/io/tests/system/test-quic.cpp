...
TEST(QuicNativeBackendTest, PreConnectionOperationsAreStableAndReportFailures) {
    ...
    EXPECT_THROW(
        {
            auto stream_id = backend->open_stream(qb::io::quic::stream_direction::bidirectional);
            (void) stream_id;
        },
        std::runtime_error);
...
TEST(QuicNativeBackendTest, ServerParentNoConnectionPathsRemainOpen) {
    ...
    EXPECT_THROW(
        {
            auto stream_id = backend->open_stream(99, qb::io::quic::stream_direction::bidirectional);
            (void) stream_id;
        },
        std::runtime_error);
...
TEST(QuicEndpointTest, DelegatesClientLifecycleToBackend) {
    ...
    qb::io::async::quic::endpoint endpoint{std::unique_ptr<qb::io::quic::backend>(raw_backend)};
...
TEST(QuicEndpointTest, PassesBackpressureAndLifecycleSettingsToBackend) {
    ...
    qb::io::async::quic::endpoint endpoint{std::unique_ptr<qb::io::quic::backend>(raw_backend), settings};
...
TEST(QuicEndpointTest, DispatchesBackendEventsToDerivedCallbacks) {
    auto              *raw_backend = new FakeQuicBackend;
    CallbackQuicClient client{std::unique_ptr<qb::io::quic::backend>(raw_backend)};

    ASSERT_TRUE(client.connect(qb::io::uri{"quic://127.0.0.1:4433"}));
    raw_backend->queued_events.push_back({qb::io::quic::backend_event::kind::connected, 0, 0, 0, "h3", {}});
    raw_backend->queued_events.push_back({qb::io::quic::backend_event::kind::stream_started, 0, 0, 0, {}, {}});
    client.poll();
    ...
    raw_backend->queued_events.push_back({qb::io::quic::backend_event::kind::connection_closed, 0, 0, 42, "done", {}});
    client.poll();
...
TEST(QuicEndpointTest, BaseEndpointAcceptsAllBackendEventsAsNoopCallbacks) {
    ...
    raw_backend->queued_events.push_back({qb::io::quic::backend_event::kind::connected, 3, 0, 0, "h3", {}});
    raw_backend->queued_events.push_back({qb::io::quic::backend_event::kind::stream_started, 3, 0, 0, {}, {}});
    ...
    raw_backend->queued_events.push_back({qb::io::quic::backend_event::kind::stream_data_acked, 3, 0, 2, {}, {}});
...
TEST(QuicEndpointTest, StreamStartedEventsExposeDirectionAndOriginForClientRole) {
    ...
    raw_backend->queued_events.push_back({qb::io::quic::backend_event::kind::stream_started, 9, 0, 0, {}, {}});
    client.poll();
    ...
    raw_backend->queued_events.push_back({qb::io::quic::backend_event::kind::stream_started, 9, 1, 0, {}, {}});
    client.poll();
    EXPECT_EQ(client.last_stream_direction, qb::io::quic::stream_direction::bidirectional);
    EXPECT_EQ(client.last_stream_origin, qb::io::quic::stream_origin::remote);

    raw_backend->queued_events.push_back({qb::io::quic::backend_event::kind::stream_started, 9, 2, 0, {}, {}});
    client.poll();
    EXPECT_EQ(client.last_stream_direction, qb::io::quic::stream_direction::unidirectional);
    EXPECT_EQ(client.last_stream_origin, qb::io::quic::stream_origin::local);

    raw_backend->queued_events.push_back({qb::io::quic::backend_event::kind::stream_started, 9, 3, 0, {}, {}});
    client.poll();
    EXPECT_EQ(client.last_stream_direction, qb::io::quic::stream_direction::unidirectional);
    EXPECT_EQ(client.last_stream_origin, qb::io::quic::stream_origin::remote);
}
...
TEST(QuicEndpointTest, ConnectorWithoutStreamSessionDispatchesAllEventKinds) {
    ...
    raw_backend->queued_events.push_back({qb::io::quic::backend_event::kind::stream_data_acked, 9, 4, 123, {}, {}});
...
TEST(QuicEndpointTest, LocalStreamSessionCanPublishAndFlush) {
    auto              *raw_backend = new FakeQuicBackend;
    CallbackQuicServer server{std::unique_ptr<qb::io::quic::backend>(raw_backend)};
...
TEST(QuicEndpointTest, RemoteStreamDataFeedsSessionExtendsCreditAndFlushesResponse) {
    auto          *raw_backend = new FakeQuicBackend;
    EchoQuicServer server{std::unique_ptr<qb::io::quic::backend>(raw_backend)};
...
TEST(QuicEndpointTest, DispatchesStreamAckEvents) {
    ...
    raw_backend->queued_events.push_back({qb::io::quic::backend_event::kind::stream_data_acked, 7, 11, 42, {}, {}});
...
TEST(QuicEndpointTest, NativeClientAndServerCompleteLocalHandshake) {
    ...
    const auto datagram_deadline = std::chrono::steady_clock::now() + std::chrono::seconds(3);
    while (server.datagram_received != "capsule" && std::chrono::steady_clock::now() < datagram_deadline) {
        qb::io::async::run(EVRUN_NOWAIT);
        std::this_thread::sleep_for(std::chrono::milliseconds(1));
    }
...
TEST(QuicEndpointTest, NativeAlpnMismatchDoesNotEstablishConnection) {
    ...
    const auto deadline = std::chrono::steady_clock::now() + std::chrono::seconds(3);
    while (client.current_state() != qb::io::async::quic::endpoint::state::closed && std::chrono::steady_clock::now() < deadline) {
        qb::io::async::run(EVRUN_NOWAIT);
        std::this_thread::sleep_for(std::chrono::milliseconds(1));
    }
...
TEST(QuicEndpointTest, NativeServerRoutesMultipleClientsByConnectionId) {
    ...
    const auto stream_deadline = std::chrono::steady_clock::now() + std::chrono::seconds(3);
    while ((server.received.find("first-client") == std::string::npos || server.received.find("second-client") == std::string::npos)
    ...
    while ((!server.is_open() || server.stats().active_connections != 1u) && std::chrono::steady_clock::now() < close_deadline) {
        qb::io::async::run(EVRUN_NOWAIT);
        std::this_thread::sleep_for(std::chrono::milliseconds(1));
    }
