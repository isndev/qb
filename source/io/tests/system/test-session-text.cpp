...
struct unix_socket_file {
    explicit unix_socket_file(std::string_view suffix)
        : path("/tmp/qb-session-text-" + std::to_string(::getpid()) + "-" + std::string(suffix) + ".sock") {
        ::unlink(path.c_str());
    }
...
TEST(Session, COMMAND_OVER_TCP) {
    ...
    std::thread t([]() {
        async::init();
        TestClient client;
        ASSERT_EQ(client.transport().connect_v4("127.0.0.1", COMMAND_TCP_PORT), SocketStatus::Done);
        client.start();
...
TEST(Session, COMMAND_OVER_UTCP) {
    unix_socket_file socket_path{"command-utcp"};
    async::init();
    msg_count_server_side = 0;
    msg_count_client_side = 0;

    TestServer server;
    ASSERT_EQ(server.transport().listen_un(socket_path.path), SocketStatus::Done);
    server.start();

    std::thread t([path = socket_path.path]() {
        async::init();
        TestClient client;
        if (SocketStatus::Done != client.transport().connect_un(path)) {
...
public:
    ...
    on(qb::io::async::event::disconnected const &ev) {
        std::fprintf(stderr, "[UTCP DIAG] server-session disconnected reason=%d sysErr=%d msgServerRecv=%zu pendingWrite=%zu\n", ev.reason,
                     this->system_error(), msg_count_server_side.load(), static_cast<std::size_t>(this->has_pending_write() ? 1 : 0));
        std::fflush(stderr);
    }
};
...
public:
    ...
    void
    on(qb::io::async::event::disconnected const &ev) {
        std::fprintf(stderr, "[UTCP DIAG] client disconnected reason=%d sysErr=%d msgRecv=%zu pendingWrite=%zu\n", ev.reason,
                     this->system_error(), msg_count_client_side.load(), static_cast<std::size_t>(this->has_pending_write() ? 1 : 0));
        std::fflush(stderr);
    }
};
...
TEST(Session, COMMAND_OVER_SECURE_TCP) {
    ...
    std::thread tc([]() {
        ...
        for (std::size_t i = 0; i < NB_CLIENTS; ++i) {
            ...
            if (SocketStatus::Done != client.transport().connect_v4("127.0.0.1", COMMAND_SECURE_TCP_PORT)) {
                throw std::runtime_error("could not connect");
            }
            client.start();
    ...
    EXPECT_TRUE(pump_until([]() { return server_done() && client_done(); }, std::chrono::seconds(10)));
    tc.join();
...
TEST(Session, COMMAND_OVER_SECURE_UTCP) {
    unix_socket_file socket_path{"secure-command-utcp"};
    async::init();
    msg_count_server_side = 0;
    msg_count_client_side = 0;
    const auto cert_file  = ssl_resource_path("cert.pem");
    ...
    ASSERT_EQ(server.transport().listen_un(socket_path.path), SocketStatus::Done);
    server.start();
    std::thread tc([path = socket_path.path]() {
        async::init();
        for (std::size_t i = 0; i < NB_CLIENTS; ++i) {
            msg_count_client_side = 0;
            TestSecureClient client;
            ...
            if (!pump_until(client_done, std::chrono::seconds(10))) {
                std::fprintf(stderr, "[UTCP DIAG] client thread iter %zu timeout: client=%zu/%zu server=%zu (expected %zu)\n", i,
                             msg_count_client_side.load(), NB_ITERATION, msg_count_server_side.load(), NB_ITERATION * (i + 1));
                std::fflush(stderr);
                ADD_FAILURE();
            }
        }
    ...
    if (!pump_until([]() { return server_done() && client_done(); }, std::chrono::seconds(10))) {
        std::fprintf(stderr, "[UTCP DIAG] main timeout: server=%zu/%zu client=%zu/%zu\n", msg_count_server_side.load(),
                     NB_ITERATION * NB_CLIENTS, msg_count_client_side.load(), NB_ITERATION);
        std::fflush(stderr);
        ADD_FAILURE();
    }
    tc.join();
...
bool
bin_all_done() {
    return bin_msg_count_server >= BIN_ITERATIONS && bin_msg_count_client >= BIN_ITERATIONS;
}
...
TEST(Session, BINARY16_OVER_TCP) {
    ...
    std::thread t([]() {
        async::init();
        BinaryClient client;
        if (SocketStatus::Done != client.transport().connect_v4("127.0.0.1", BINARY16_TCP_PORT)) {
            throw std::runtime_error("could not connect");
        }
        client.start();
...
TEST(Session, PROTOCOL_SWITCH_TEXT_TO_BINARY) {
    ...
    std::thread       t([&done]() {
        ...
        ASSERT_EQ(sock.connect_v4("127.0.0.1", PROTO_SWITCH_TCP_PORT), SocketStatus::Done);
...
class TextProtocolQuicClient : public use<TextProtocolQuicClient>::quic::connector<TextQuicSession> {};
