...
TEST_F(StreamTest, DISABLED_MemoryBufferStream) {
    ...
    public:
        MemoryBuffer() : _buffer(1024, 0) {}
        
        // Copy constructor needed for assignment
        MemoryBuffer(const MemoryBuffer& other) 
            : _buffer(other._buffer), _read_pos(other._read_pos), _write_pos(other._write_pos) {}
        ...
        int read(char* data, std::size_t size) {
            if (_read_pos >= _write_pos) return 0; // EOF
...
TEST_F(StreamTest, StreamPerformance) {
    ...
    std::cout << "Stream read performance: " << total_bytes << " bytes in " 
              << duration << "ms (" 
              << (total_bytes / (duration ? duration : 1)) << " bytes/ms)" 
              << std::endl;
...
public:
    TransformStream(BaseStream& base, std::function<void(char*, size_t)> transform)
        : _base_stream(base), _transform_func(transform) {}
    ...
    auto& in() { return _base_stream.in(); }
};
