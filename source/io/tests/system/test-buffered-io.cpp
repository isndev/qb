...
public:
    ...
    qb::allocator::pipe<char>       &in() noexcept { return _in; }
    qb::allocator::pipe<char> const &in() const noexcept { return _in; }
    qb::allocator::pipe<char>       &out() noexcept { return _out; }
    qb::allocator::pipe<char> const &out() const noexcept { return _out; }
    ...
    void set_max_write_buffer_size(std::size_t size) noexcept {
    ...
    void flush(std::size_t size) noexcept { _in.free_front(size); }

    void reset_state() noexcept { reset_buffered_io_state(); }

    void on(qb::io::async::event::pending_read &&event) noexcept {
        ++pending_read_events;
        last_pending_read = event.bytes;
    }

    void on(qb::io::async::event::eof &&) noexcept { ++eof_events; }

    void on(qb::io::async::event::disconnected &&event) noexcept {
        ++disconnected_events;
        last_disconnect_reason = event.reason;
    }

    void on(qb::io::async::event::dispose &&) noexcept { ++dispose_events; }
};
...
public:
    FixedFrameProtocol(BufferedProbe &io, std::size_t frame_size,
                       bool close_after_message = false) noexcept;
...
public:
    ...
    std::size_t getMessageSize() noexcept final { return _reported_size; }
    void        onMessage(std::size_t) noexcept final { ADD_FAILURE(); }
    void        reset() noexcept final {}
};
...
public:
    ...
    std::size_t getMessageSize() noexcept final { return 0u; }
...
FixedFrameProtocol::FixedFrameProtocol(BufferedProbe &io, std::size_t frame_size,
                                       bool close_after_message) noexcept
    : AProtocol(io)
...
struct ServerDisposeRecorder {
    ...
    void disconnected(std::uint64_t connection_id, std::uint64_t stream_id) noexcept {
        ++disconnected_calls;
...
class ServerOwnedBufferedProbe
    : public qb::io::async::buffered_io<ServerOwnedBufferedProbe> {
    qb::allocator::pipe<char> _in;
    qb::allocator::pipe<char> _out;
...
public:
    ...
    [[nodiscard]] std::uint64_t id() const noexcept { return 42u; }
    [[nodiscard]] std::uint64_t connection_id() const noexcept { return 7u; }
    ServerDisposeRecorder &server() noexcept { return _server; }
};
...
public:
    ...
    void append(std::string_view data) { _in << data; }
};
...
class LineTerminatedProtocol
    : public qb::protocol::base::byte_terminated<ProtocolProbe, '\n'> {
public:
...
class HeaderTerminatedProtocol
    : public qb::protocol::base::bytes_terminated<ProtocolProbe, DoubleCrLf> {
public:
...
class SizeHeaderProtocol
    : public qb::protocol::base::size_as_header<ProtocolProbe, Size> {
public:
    using qb::protocol::base::size_as_header<ProtocolProbe,
                                             Size>::size_as_header;
