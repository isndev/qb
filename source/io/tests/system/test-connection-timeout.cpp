...
TEST_F(ConnectionTimeoutTest, TCPConnectionTimeout) {
    ...
    if (result > 0) {
        ...
        try {
            ...
            EXPECT_FALSE(static_cast<bool>(peer_ep)) << "Socket shouldn't be connected to an unreachable peer";
...
TEST_F(ConnectionTimeoutTest, UDPDatagramTimeout) {
    ...
    int result = qb::io::socket::recv_n(socket.native_handle(), buffer, sizeof(buffer), std::chrono::microseconds(timeout));
    ...
    if (result < 0) {
        EXPECT_TRUE(errno == EWOULDBLOCK || errno == EAGAIN || errno == ETIMEDOUT || errno == EINTR || errno == EINPROGRESS);
    }
}
