...
macro(config_compiler_and_linker)
    ...
    elseif (CMAKE_COMPILER_IS_GNUCXX)
        set(cxx_base_flags "-Wall -Wno-interference-size -Wno-narrowing -Wno-sign-compare -Wno-changes-meaning -march=${${QB_PREFIX_UPPER}_BUILD_ARCH}")
...
# with one of them.
function(cxx_library_with_type type)
    #  if (CMAKE_VERBOSE_MAKEFILE)
    #    message("Build library ${name} with flags: ${Library_FLAGS}")
    #  endif()
    set(options NONE)
    set(oneValueArgs NAME FLAGS)
    ...
    if (Library_NAME)
        ...
        add_library(${Library_NAME} ${type} ${Library_SOURCES})
        message(STATUS "INFO: Generating ${Library_NAME} library as ${type}")
        ...
        if (NOT type STREQUAL "INTERFACE")
            ...
            get_target_property(pdb_debug_postfix ${Library_NAME} DEBUG_POSTFIX)
            set_target_properties(${Library_NAME}
                    PROPERTIES
                    PDB_NAME "${Library_NAME}"
                    PDB_NAME_DEBUG "${Library_NAME}${pdb_debug_postfix}"
            ...
            if (type STREQUAL "SHARED")
                set_target_properties(${Library_NAME}
                        PROPERTIES
                        COMPILE_FLAGS "${Library_FLAGS}")
                target_compile_definitions(${Library_NAME} PUBLIC ${QB_PREFIX_UPPER}_DYNAMIC=1)
                if (NOT "${CMAKE_VERSION}" VERSION_LESS "2.8.11")
                    target_compile_definitions(${Library_NAME} INTERFACE ${QB_PREFIX_UPPER}_LINKED_AS_SHARED=1)
                endif ()
            endif ()
            if (DEFINED ${QB_PREFIX_UPPER}_HAS_PTHREAD)
...
#                target_link_libraries(${Library_NAME} PUBLIC ${threads_spec})
                list(APPEND Library_DEPENDENCIES ${threads_spec})
            endif ()
            if (Library_DEPENDENCIES)
                #message(STATUS "INFO: Generating ${Library_NAME} library dep ${Library_DEPENDENCIES}")
                if (Library_SOURCES)
...
function(cxx_interface_library)
    cxx_library_with_type("INTERFACE" ${ARGN})
endfunction()

function(cxx_shared_library)
    cxx_library_with_type("SHARED" ${ARGN})
endfunction()

function(cxx_library)
    cxx_library_with_type("STATIC" ${ARGN})
endfunction()
