...
public:
    ...
    stream(std::uint64_t id, qb::io::quic::stream_direction direction, qb::io::quic::stream_origin origin) noexcept
        : stream(0, id, direction, origin) {}

    stream(std::uint64_t connection_id, std::uint64_t id, qb::io::quic::stream_direction direction, qb::io::quic::stream_origin origin) noexcept
    ...
    void
    reset(std::uint64_t /*application_error_code*/ = 0) {
        close();
    }
...
public:
    ...
    void
    set_max_read_buffer_size(std::size_t value) noexcept {
        _max_read_buffer_size = value;
    }

    void
    set_max_write_buffer_size(std::size_t value) noexcept {
        _max_write_buffer_size = value;
    }

    void
    assign_stream_id(std::uint64_t stream_id) noexcept {
        _stream_id = stream_id;
        _open      = true;
    }

    void
    assign_connection_id(std::uint64_t connection_id) noexcept {
        _connection_id = connection_id;
    }

    void
    assign_quic_ids(std::uint64_t connection_id, std::uint64_t stream_id) noexcept {
        _connection_id = connection_id;
        _stream_id     = stream_id;
        _open          = true;
    }
    ...
    append(std::string_view data) {
        ...
        if (max_read != static_cast<std::size_t>(-1) && pendingRead() + data.size() > max_read) {
    ...
    bool
    append(std::span<const std::byte> data) {
        return append(std::string_view{reinterpret_cast<const char *>(data.data()), data.size()});
    }

    char *
    publish(char const *data, std::size_t size) {
        if (!_open)
            return nullptr;
    ...
    attach_protocol_if_declared() {
        if constexpr (qb::has_type_Protocol<Derived>) {
            if constexpr (!std::is_void_v<typename Derived::Protocol>) {
                this->template switch_protocol<typename Derived::Protocol>(static_cast<Derived &>(*this));
            }
        }
    }
};
