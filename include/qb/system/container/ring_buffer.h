...
namespace qb {
     ...
     * @tparam Overwrite Whether to overwrite old elements when the buffer is full (defaults to true)
     */
     ...
     * either overwrite the oldest ones (if Overwrite=true) or are discarded (if Overwrite=false).
     * 
     * @tparam T The type of elements stored in the ring buffer
     * @tparam N The capacity of the ring buffer
     * @tparam Overwrite Whether to overwrite old elements when the buffer is full (defaults to true)
    ...
    public:
         ...
         * 
         * If the buffer is full and Overwrite is true, the oldest element will be overwritten.
         * If the buffer is full and Overwrite is false, the element will be discarded.
         * 
         ...
         */
        [[nodiscard]] reference back() noexcept { return reinterpret_cast<reference>(elements_[std::clamp<size_type>(head_ - 1, 0UL, N - 1)]); }
