<!-- Verified-against: qb 3.2.0 (C++20 default, C++23 supported). Source of truth: the headers and sources under qb/src. -->
...
any path** — the runtime relocates events with raw `memcpy` (`reply`/`forward` byte-recycle it, a
cross-core hop copies it twice) and a
...
in-place population — that reference lives until the handler that obtained it **returns** (the
pipe is segmented and never moves a queued event; further pushes are fine, a `co_await` or a
member is not);
...
it at poll latency and a callback armed on it fires at its delay — `latency` only caps the park (the wait is as fine as the backend: nanoseconds under kqueue and under epoll on Linux ≥ 5.11 via `epoll_pwait2`, whole milliseconds under io_uring and Windows' wepoll, where a sub-millisecond timer or `latency` is met at the millisecond); with
no io watchers it parks on its mailbox condition variable and wakes for a producer's event or the
timeout. Either park ends the moment another core enqueues an event to it.
...
qb::io::async::callback([] { /* captures NOTHING from an actor */ }, 100ms);
// delay <= 0 (or the no-duration overload) executes the lambda inline, NOT next iteration.

// NEVER capture `this` in the TIMED overload from an actor. The Timeout is owned by the event
// loop, not by the actor, so it fires after ~Actor(); and `if (is_alive())` inside it does NOT
// help -- is_alive() is a member read, so the guard IS the use-after-free. AddressSanitizer
// reports it as heap-use-after-free. From an actor, wait like this instead:
spawn([d = 100ms](qb::ScopedCoroContext ctx) -> qb::io::async::task<void> {
    co_await ctx.sleep(d);                    // cancelled when the actor is killed
    ctx.template push<TickEvent>();           // work happens in on(TickEvent&), on a LIVE actor
});
// The one-argument overload is different and safe to capture `this` in: it runs INLINE, so the
// actor cannot have gone away between the call and the body.
...
socket. `spawn()` hands the lambda a `qb::ScopedCoroContext`. _(Actor.h:1396-1431)_
...
- **After any `co_await`, the only legal channel back is the context.** `ctx.push<E>()` /
  ...
  dropped, not delivered into freed memory. _(Actor.h:1586-1592)_
...
`co_await qb::ask(context(), target, req, 500ms)`. _(Actor.h:1433-1449, :2211-2214)_
...
`CoroContext` there is a compile error, not a runtime risk. _(Actor.h:1336-1394)_
...
- **Send to self / schedule** — `push<TickEvent>(id())` now, or, after a delay,
  `spawn([d](qb::ScopedCoroContext ctx) -> qb::io::async::task<void> { co_await ctx.sleep(d);
  ctx.template push<TickEvent>(); })`. Do **not** use `qb::io::async::callback([this]..., delay)`
  for this: that timer outlives the actor and an `is_alive()` re-check inside it is itself the
  use-after-free.
...
- **`onInit()` is an async coroutine (`qb::io::async::task<bool>`) that may `co_await`; it must
  ...
  is *Activating*. _(Actor.h:459-472)_
- **`push`/`send`/`broadcast` and the messaging hot path are `noexcept`.** A throw across that boundary
  ...
  small and allocation-light. _(Actor.h:1063-1069; Pipe.h:138-153)_
- **`send<T>()` is unordered; `push<T>()` is ordered.** Trivial destructibility is a guideline on `send`
  ...
  24.9, measured in the qb-vs-others benchmark, TUNING guide §9.6). _(Actor.h:1018-1023, :1078-1082)_
...
- **The idle-spin floor (`setIdleSpin`, 50 µs) is a policy, not a mechanism.** A core that really
  ...
  the other way; `setLatency(0)` never sleeps at all. _(Main.h:300, :317, :366-368; measured in the
...
- **A `setLatency` under a millisecond sleeps a millisecond and a half on Windows, and what it says
  ...
  rounds to whole milliseconds) and the loop of a core that owns one (wepoll's `epoll_wait` takes
  whole milliseconds, and the kernel adds its coalescing on top) — wake an idle core every 1.4–1.5 ms
  at `setLatency(100us)`, 1.4–1.8 ms at `1ms`, 10.5 ms at `10ms` (measured: an idle core's wakes per
  second). `setLatency(1us)` and `setLatency(1ms)` sleep the same there; only `setLatency(0)` goes
  below. On Linux 5.11 or newer the same core wakes every 163 µs at `100us`, 1.07 ms at `1ms`,
  10.05 ms at `10ms` through either park — the kernel's 50 µs timer slack plus a pass (the loop park
  through `epoll_pwait2`; an older kernel keeps `epoll_wait`'s whole millisecond for that park, the
  condition variable stays fine). _(measured in the qb-vs-others benchmark, TUNING guide §19)_
...
- **`addRefActor<T>()` returns a phase-aware `qb::ActorHandle<T>` (alias `RefActorHandle<T>`);**
  ...
  _(Actor.h:1290-1294, :1314, :2279-2281)_
- **`getService<T>()` is the ONE lookup that is not phase-gated: it hands back a service whose async
  ...
  the inventory table Actor.h:818-862)_
...
- **Configure cores/actors before `start()`.** `Main::core()` throws once the engine is running. A core
  with 0 actors fails startup. _(Main.cpp:607-609, :425-427)_
- **`Actor::time()` is the VirtualCore's cached nanosecond timestamp,** constant within one handler /
  ...
  `qb::unix_nanos(qb::wall_now())`. _(Actor.h:753-769; VirtualCore.h:867-879; VirtualCore.cpp:1228-1235)_
...
- **`file_watcher`/`directory_watcher` own their watched path string.** qev's `ev_stat` stores the path
  **pointer** without copying, so the watcher keeps a `std::string _watched_path` alive for its lifetime — never
  hand `ev::stat` a temporary's `c_str()`. _(io.h:576-579; ev++.h:762)_
