...
*   `[T<E:ask_event_type, Args...>] [[nodiscard]] task<E> ask(ScopedCoroContext ctx, ActorId target, qb::duration timeout, Args... args)` — **emplace** form, `E` explicit: the request is constructed from `args` directly in the outgoing pipe slot (no caller temporary, no copy — the by-value form moves a ≥ 64-byte event three times and stalls on its own header). `Args` by value, moved (a `task` is lazy). Same contract and exceptions as the by-value form.
