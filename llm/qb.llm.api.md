<!-- Verified-against: qb 3.2.0 (C++20 default, C++23 supported). Source of truth: the headers under qb/src. -->
...
*   `void setIdleSpin(qb::duration idle_spin = CoreInitializer::kDefaultIdleSpin)` — idle-spin floor for all cores, overriding per-core values set earlier; pre-start only.
...
*   `explicit Actor(no_default_events_t) noexcept` (protected) — registers NO default events; derived must register at least **`SignalEvent`** in `onInit()` (that is what `Main::stop()` and SIGINT/SIGTERM arrive as; the engine never sends a `KillEvent`, so registering only that leaves the actor unstoppable and `Main::join()` hanging). Add `KillEvent` as well if a peer will push one.
...
*   `static void *operator new(std::size_t)`, `operator new(std::size_t, std::align_val_t)`, placement `operator new(std::size_t, void*) noexcept`, and the matching sized `operator delete(void*, std::size_t) noexcept`, aligned and placement forms — the actor object comes from the core thread's `qb::allocator::thread_arena` (16-byte size classes up to 1 KiB over `slab_cache` slabs, LIFO reuse per class; a type above 1 KiB or with `alignas` above 16 falls through to the global allocator). A derived class that declares its own `operator new` / `operator delete` hides them (the pool hook); `qb::allocate_actor<T>` stays the construction hook. Never `delete` an actor yourself _(`Actor.h:534-555`)_.
...
*   `[T<_ServiceActor>] [[nodiscard]] _ServiceActor* getService() const noexcept` — same-core service pointer, or `nullptr` if no service of that type is registered on this core. **The one lookup that is NOT phase-gated**, deliberately: it consults neither `is_active()` nor `is_alive()`, so it resolves a service whose async `onInit()` is still in flight *and* one that has been killed but not yet reaped. That is what lets a service look itself or a peer up from inside its own `onInit()` — the answer to "can I look this up from `onInit()`?" is yes here and **no** for `ActorHandle::get()`. The cost is yours: what you get back may be mid-init, so `push` it an event rather than reading its state _(`Actor.h:785-802`, body `VirtualCore.h:974-997`)_.
*   `[[nodiscard]] bool is_alive() const noexcept` — true until `kill()` processed. **Not** a phase check: it is already true while the actor is still Activating.
*   `[[nodiscard]] bool is_active() const noexcept` — alive **and** past `onInit()`. It differs from `is_alive()` exactly across the Activating window, so it is the self-directed counterpart of `ActorHandle::ready()` (for a child) and `is_actor_alive(id)` (for a peer). Exactly two lookups consult it — `VirtualCore::findActor<T>()` (hence every `ActorHandle` accessor) and `VirtualCore::isActorAlive()`; `getService<T>()` consults nothing, and the inbound-event dispatch gate keys off `VirtualCore::_activating` membership instead and **defers** rather than withholds. The header carries the full inventory as a table _(`Actor.h:818-862`)_.
...
*   `[T<_Event,_Args...>] void send(const ActorId& dest, _Args&&... args) const noexcept` — **unordered** fire-and-forget. Trivial destructibility is a guideline here and a compiler-enforced rule only when `_Event` derives from `qb::EventQOS0`, the one kind the cross-core flush may DROP undisposed; a delivered event is disposed exactly once whichever primitive queued it, `send` included. Prefer `push()`.
...
*   `[T<_Actor,_Args...>] [[nodiscard]] ActorHandle<_Actor> addRefHandle(_Args&&... args) const` — **a pure alias of `addRefActor<T>()`**: it forwards to it and returns the same phase-aware `qb::ActorHandle<T>`. It is not a different or "more checked" handle. Retained for source compatibility; new code can just call `addRefActor` _(`Actor.h:1317-1327`)_.

Coroutines (C++20) — `spawn` is the default; `spawn_detached` is the low-level escape hatch:
*   `[T<Func>] void spawn(Func&& func) const` — **preferred**. Runs `func(qb::ScopedCoroContext)` bound to the actor's per-actor cancellation scope: killing/destroying the actor cancels the scope, so a coroutine parked on `ctx.sleep` / `ctx.cancellation_point` / `ctx.until_cancelled` / `ctx.cancellable` / any patterns-library helper wakes on the next loop iteration, throws `qb::io::async::cancelled_error`, and unwinds cleanly (RAII + `catch` run). Must be called from the actor's own VirtualCore thread. Pass the lambda **without** trailing `()`. An exception OTHER than `cancelled_error` escaping the body is caught by the wrapper and REPORTED on `std::cerr` (naming the actor, the API and `what()`) — it is not propagated anywhere, because a spawned coroutine has no caller; catch it in the body and answer through an event. _(`spawn` declared `Actor.h:1396-1431`, body `VirtualCore.h:1446`)_
*   `[T<Func>] void spawn_detached(Func&& func) const` — low level. Runs `func(qb::CoroContext)` in an isolated context that is **not** cancelled on kill: it runs to completion, orphaned. Use only for fire-and-forget work that must intentionally outlive the actor. It cannot drive `qb::ask` or any patterns helper (they require a `ScopedCoroContext`; a `CoroContext` is a compile error). _(`spawn_detached` declared `Actor.h:1336-1394`, body `VirtualCore.h:1432`)_
...
*   `[[nodiscard]] bool resolve_require(RequireEvent& e) const noexcept` — the discovery counterpart of `resolve_ask`. The auto-registered `void on(RequireEvent&) noexcept` already calls it, so `co_await qb::ping` / `qb::require` need no boilerplate; an override of that handler must call it first or those coroutines never resolve. Returns `false` for a legacy `correlation_id == 0` reply or one that is not ours.
*   `[[nodiscard]] bool has_active_coroutines() const`, `[[nodiscard]] std::size_t active_coroutine_count() const`, `[[nodiscard]] bool has_coro_scope() const noexcept` (true once `spawn()` **or** `context()` lazily allocated the scope; `spawn_detached()` never does — `VirtualCore.h:1432-1433` vs `:1446-1448`).
...
Safe restricted context handed to `spawn_detached` coroutines (captures the actor's `ActorId` by value, survives destruction — events to a dead actor are dropped, never delivered into freed memory). The `CoroContext` class is at _(`Actor.h:1591-1600`)_; the alias `qb::coro_context` at _(`Actor.h:2701`)_.
...
#### `class qb::ActorHandle<_Actor>` — canonical; `qb::RefActorHandle<_Actor>` is an alias of it
Type-safe, **phase-aware** weak reference to a same-core referenced actor, returned by `addRefActor` / `addRefHandle`. `RefActorHandle` is not a class: it is a `using` alias retained for source compatibility _(`Actor.h:2389-2390`)_. Dereference only on the owning core's thread.
*   `[[nodiscard]] ActorId id() const noexcept` — valid the instant `addRefActor` returns, **even while the child is still Activating**, so it is always safe to `push()` to (events are stashed and replayed FIFO once the child becomes active).
*   `[[nodiscard]] bool valid() const noexcept` — true iff the handle was constructed from a non-null actor (allocation succeeded). **Not** a liveness or readiness test.
*   `[[nodiscard]] _Actor* get() const noexcept` — the pointer iff resolved **and** `is_active()`. `nullptr` otherwise — and "otherwise" is wider than "dead": also **while the child is still Activating** (its async `onInit()` has not completed) and after a failed init _(`Actor.h:2317-2323`)_.
*   `[[nodiscard]] bool ready() const noexcept` — `== (get() != nullptr)`; the sync gate for the Activating window _(`Actor.h:2325-2329`)_.
*   `[[nodiscard]] qb::io::async::task<bool> ready_async(ScopedCoroContext ctx, qb::duration timeout = std::chrono::seconds{5}) const` — `co_await` until the child is active — event-driven since 3.2: resumed by the core pass that ends the child's activation (after its stashed events are replayed), not by a 1 ms poll; returns `false` if it did not become active in time, or as soon as it is known that it never will (a failed or throwing init, the activation deadline, a kill while Activating); an already-active child answers without suspending; a kill of the waiting actor throws `cancelled_error`. The async gate for the same window _(`Actor.h:2355-2358`; the awaiter `Actor.h:2000-2104`)_.
*   `operator->`, `operator*` — `get()` with a **debug assert** that it is ready; in a release build an unready handle is UB.
*   `explicit operator bool` — `== ready()`.
*   **Trap:** `auto h = addRefActor<Child>(cfg); h->method();` dereferences `nullptr` whenever `Child::onInit()` is async — debug assert, release UB. Gate the dereference on `ready()`, or `if (co_await h.ready_async(context())) h->method();`. Only the dereference needs the gate; `push(h.id(), ...)` never does.
...
*   `[[nodiscard]] bool is_alive() const noexcept` — the reuse marker `reply()`/`forward()` set on the ORIGINAL after copying it out; **false** on a freshly built event and on every copy a transport delivers (the copy clears it), so the dispatcher destroys exactly the copy that stayed.
...
#### QoS tiers
*   `using EventQOS2 = Event;`, `using EventQOS1 = Event;` — the SAME type as each other and as the base
...
*   `[T<_Actor,_Args...>] [[nodiscard]] _Actor* qb::allocate_actor(_Args&&... args)` — ADL customization point for actor CONSTRUCTION (default `new _Actor(args...)`, which reaches `Actor::operator new` and so the arena below); a pool that must own the bytes hides `operator new` / `operator delete` in the actor class instead.
*   `class qb::allocator::thread_arena` (`<qb/system/allocator/thread_arena.h>`) — the per-thread size-class arena `Actor::operator new` draws from, all members static: `[[nodiscard]] static void *allocate(std::size_t size)` / `allocate(std::size_t size, std::size_t align)`, `static void deallocate(void *p, std::size_t size) noexcept` / the aligned form, `live()`, `chunks()`, `chunk_bytes()`, `orphaned()`; constants `granule` (16), `max_small` (1024), `classes` (64), `first_chunk_bytes` (64 KiB). A block is freed on the thread that allocated it (nothing checks it; `qb::Actor` enforces it one level up); chunks come from `::operator new` (the first, 64 KiB) then `slab_cache` (2 MB slabs), go back to the cache when the thread exits with nothing live, and are ORPHANED — kept, reachable, counted, never recycled — when it exits with a live block _(`thread_arena.h:71-179`)_.
...
*   `[T<E:ask_event_type>] [[nodiscard]] ask_operation<E> ask(ScopedCoroContext ctx, ActorId target, E req, qb::duration timeout)` — send + co_await the reply; throws `timeout_error` / `cancelled_error` (kill). Returns the exchange as an awaitable that lives in YOUR frame (no coroutine frame of its own, 3.2): `co_await` it, or let it convert to a `task<E>`.
*   `[T<E:ask_event_type, Args...>] [[nodiscard]] ask_emplace_operation<E, Args...> ask(ScopedCoroContext ctx, ActorId target, qb::duration timeout, Args... args)` — **emplace** form, `E` explicit: the request is constructed from `args` directly in the outgoing pipe slot (no caller temporary, no copy — the by-value form moves a ≥ 64-byte event twice and stalls on its own header). `Args` by value, moved (the operation is lazy). Same contract and exceptions as the by-value form.
*   `[T<E:ask_event_type>] class ask_operation<E>` — what `ask` returns: the context, the target, the request and the timeout, plus the `detail::ask_awaiter` engaged in place by `await_suspend()`; a prvalue built into the awaiting frame. Lazy — nothing is sent until it is `co_await`ed; `await_ready()` is the scope's cancel flag, so a cancelled scope sends nothing and `await_resume()` throws `cancelled_error` at once; `co_await` yields `E` with one move. `value_type`, `task_type`; `operator task<E>() &&` (implicit) for `task<E> t = ask(...)` and `std::vector<task<E>>::emplace_back(ask(...))`; movable until awaited, never copyable. Never constrain on `ask` returning exactly `task<E>`. _(request.h:110)_
*   `[T<E:ask_event_type, Args...>] class ask_emplace_operation<E, Args...>` — the emplace form's operation: holds the constructor arguments and builds `E` in the pipe slot at `await_suspend()`; same contract, aliases and conversion. _(request.h:170)_
...
*   `[T<E, Args...>] [[nodiscard]] task<E> ask_by(ScopedCoroContext ctx, ActorId target, deadline dl, Args... args)` — emplace form of `ask_by`, same deadline contract.
...
*   `[T] class growable_ring<T>` — growable single-thread FIFO ring (`<qb/system/container/growable_ring.h>`): power-of-two storage aligned for `T`, doubled when full (elements moved), no allocation per element; `emplace_back`/`push_back`/`pop_front`/`front`/`operator[]`/iterators/`erase(it)`/`erase_at(i)`/`clear`/`size`/`empty`/`capacity`; destroys what it holds. What `ask_stream` buffers chunks in and what `channel<T>` and the coroutine sync primitives park waiters in since 3.2 — MSVC's `std::deque` holds one element per block past 8 bytes (Huly QB-215). _(container/growable_ring.h:51)_
...
*   Free drivers:
    ...
    *   `std::size_t run_once()` — EVRUN_ONCE (waits for ≥1 event; may block). Throws the same `std::logic_error` as `run()`.
    *   `std::size_t run_until(const bool& status)` — loop while `status` (50µs idle sleep). Throws the same `std::logic_error` as `run()`.
...
*   `[T] class task<T = void>` — primary move-only coroutine return type, awaitable via `co_await`. `handle()`, `[[nodiscard]] handle_type detach() noexcept`, `done()`, `explicit operator bool`. `T await_resume()` — rethrows a stored exception; throws `std::logic_error` in **two** cases, and the first is checked first: awaited on an **empty/moved-from** task (default-constructed, moved-from, or after `detach()`), or awaited before the coroutine completed _(`task.h:736-751`)_.
...
*   the variadic `when_all` / `when_any` / `race` and `coro_with_timeout` take, besides tasks, any awaitable that converts to one — `qb::ask`'s operations — through `detail::to_task` _(combinators.h:72)_; the `std::vector<task<T>>` forms take tasks (an operation converts on `emplace_back`).
...
*   `class cancellation_token` — shared same-thread signal: `cancel()`, `is_cancelled()`, `on_cancel(cb)` → `id_type` / `remove_on_cancel(id)`, `link(cancel_hook&)` → `bool` (an EMBEDDED node, no allocation; `hook.unlink()` on normal completion — the `qb::ask` path), `throw_if_cancelled()`. Copyable: copies share an intrusively refcounted state whose count is NOT atomic, so a copy is one increment and every copy must live on the owning thread. Cross-thread cancel must go via an actor event.
