...
*   `class qb::allocator::thread_arena` (`<qb/system/allocator/thread_arena.h>`) — the per-thread size-class arena `Actor::operator new` draws from, all members static: `[[nodiscard]] static void *allocate(std::size_t size)` / `allocate(std::size_t size, std::size_t align)`, `static void deallocate(void *p, std::size_t size) noexcept` / the aligned form, `live()`, `chunks()`, `chunk_bytes()`, `orphaned()`; constants `granule` (16), `max_small` (1024), `classes` (64), `first_chunk_bytes` (64 KiB), `poisons_free_blocks` (`true` in an AddressSanitizer build only: a block on a free list is poisoned, so a read through a dangling actor pointer reports `use-after-poison`; `false` and no code elsewhere). A block is freed on the thread that allocated it (nothing checks it; `qb::Actor` enforces it one level up); chunks come from `::operator new` (the first, 64 KiB) then `slab_cache` (2 MB slabs), go back to the cache when the thread exits with nothing live, and are ORPHANED — kept, reachable, counted, never recycled — when it exits with a live block _(`thread_arena.h:93-215`)_.
