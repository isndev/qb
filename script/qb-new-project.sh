...
# ---------------------------------------------------------------------------------------------
# It produces a NEW repository: the template's payload rendered under the user's name, committed
# once, with no remote. It used to `git clone` the template and then `git remote remove origin`,
# which handed the user somebody else's commit history as their own and made their first push a
# push of it. Three structural defects were fixed together, and each had shipped:
#
...
#
#  2. IDENTIFIERS. The template shipped its own name throughout -- CMake project, target names,
#     the executable, include guards. A user who ran this got a tree that said `qb-sample-project`
#     everywhere and had to rename it by hand, which is a plausible reason nobody noticed the
#     templates had rotted: the output was never really used as-is. The template now carries
#     @QB_NAME@ / @QB_NAME_LOWER@ / @QB_NAME_UPPER@ placeholders in file contents AND in path
#     names, and this script substitutes all of them from the ONE name the user passed. The
#     vocabulary is owned here, not by the template, and an unresolved placeholder is a hard error.
#
#  3. VERSION BINDING. The template pinned qb as a submodule gitlink, and a pinned dependency
#     stored in the template can only drift -- it drifted two majors, to a commit from before
#     v2.0.0, while the template's own build stayed green against it. Nothing is stored now. The
#     generated tree fetches qb with FetchContent at a ref THIS SCRIPT WRITES, resolved from
#     QB_SHIPPED_VERSION below, which is asserted equal to qbConfig.cmake's QB_FRAMEWORK_VERSION
#     by scripts/check-scaffold-consistency.sh. Staleness is structurally impossible rather than
#     merely monitored, and the URL the one-liner is fetched from selects the pairing.
#
# ---------------------------------------------------------------------------------------------
# THE FOUR EARLIER DEFECTS, all of the same shape -- exit 0, wrong result -- kept fixed
# ---------------------------------------------------------------------------------------------
#  * No `set -e`: a failed clone let `mkdir .git; mv * .git` run against the INVOCATION directory
#    and swallow its contents, exiting 0. Reproduced end to end before it was fixed.
#  * `git --bare init` took HEAD from the user's `init.defaultBranch` while the push named the
#    template's branch, so on `init.defaultBranch=main` the script printed "Created" over a
#    directory containing nothing but `.git`. `set -e` cannot see this: no command failed. Hence
#    the anti-vacuity checks, which are what make the whole class loud.
#  * `rm -rf "../${TEMPLATE}"` was computed from a user-supplied name, so a name with a separator
#    moved the deletion onto an unrelated checkout. Every path is now resolved once, absolutely.
#  * Nothing removed the half-built target on failure, so the guards then refused the retry the
#    user obviously wanted. A trap removes only what this script created, and only on failure.
#
...
# Everything below the SHARED BODY marker is byte-identical to qb-new-module.sh. This block is
# the only part that differs, and scripts/check-scaffold-consistency.sh checks exactly that.

KIND=project
TEMPLATE=qb-sample-project

# A CMake project name and an executable name.
NAME_PATTERN='^[A-Za-z][A-Za-z0-9_-]*$'
NAME_RULE="start with a letter and contain only letters, digits, '-' or '_'"
...
qb and the qbm modules are fetched by CMake at configure time (ref ${QB_BUILD_REF}); there are no
submodules to initialise. To build against a qb checkout you already have, point CMake at it:

    cmake -S . -B build -DFETCHCONTENT_SOURCE_DIR_QB=/path/to/qb

The generated tree ships a ctest suite and a GitHub Actions workflow -- see its README.md.
EOF
}
