...
void Session::on(IOMessage message, std::size_t size) {
    ...
    std::cout << "Received from Session(" << in().ident() << ") ip(" << in().getRemoteAddress() << ")" << std::endl
              << "-> Message (" << size << "): " << std::string_view(message, size - 1) << std::endl;
...
void Session::on(qb::io::async::event::disconnected const& event) {
    std::cout << "Session(" << in().ident() << ") ip(" << in().getRemoteAddress() << ") disconnected -> ";
    switch (event.reason) {
