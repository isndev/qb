...
> **Which revision of the template you get is derived, not assumed.** The script resolves the template ref from the qb version it ships with: an explicit `QB_TEMPLATE_REF` wins, then a tag matching that qb release, then — only when the release is known not to exist yet — the development line, and finally the template's default branch, which it reports as a fallback rather than taking silently. So a scaffolded module is paired with the qb it was written for by construction, and the pairing cannot drift the way it did before 3.0, when the template carried its own qb pointer and went two major versions stale without anything noticing.
>
> The template's `master` still carries the pre-3.0 shape and does not configure against a current qb — it has no `SOURCES`, its `test/CMakeLists.txt` calls a `qb_register_module_gtest()` that does not exist (the current spelling is `qb_register_module_test()`), and its actors override `onInit()` as `bool` rather than `qb::io::async::task<bool>`. Nothing routes you there, and since 2026-08-11 that is doubly true: `master` is no longer the template's default branch (`main` is, and it carries `v3.0.0`), and the resolution above matches `v3.0.0` on its first rung, so it never reaches a development line or a default branch at all. `master` is named here only so a `git clone` of the template by hand — which now lands on `main`, not `master` — is not mistaken for what the script produces, and so that anyone who checks out the old branch deliberately knows what they have.
>
> Whatever you scaffold, a qbm module **cannot be configured standalone** — `qb_register_module()` and `qb_add_test()` are development-time helpers an installed qb does not ship. The generated tree carries a `.github/ci/superbuild/CMakeLists.txt` for exactly that reason: it is the only root from which one module builds on its own, and it is the same file, byte for byte, that `qbm-http`, `qbm-pgsql` and `qbm-redis` each ship. Those three remain the reference for the 3.0 layout.
