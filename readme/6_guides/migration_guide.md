...
- `CoreInitializer::setLatency(qb::duration latency = qb::duration::zero())` — the maximum the event loop waits when idle; the default `zero()` is the busy-spin, lowest-latency mode (100% CPU on the core), and a positive value lets the idle core park to trade latency for CPU. _(`qb/src/qb/core/Main.h:276-300`.)_
...
to the tail of **`VirtualCore.h`** (`qb/src/qb/core/VirtualCore.h:1171-1176`). Most of those bodies name
...
libev's own names — `ev_run`, `ev_io_start`, `struct ev_loop` (`qb/src/qb/ev/ev.h:709`) — because the
