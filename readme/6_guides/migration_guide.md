...
- `CoreInitializer::setLatency(qb::duration latency = qb::duration::zero())` — the maximum the event loop waits when idle; the default `zero()` is the busy-spin, lowest-latency mode (100% CPU on the core), and a positive value lets the idle core park to trade latency for CPU. _(`qb/src/qb/core/Main.h:272-286`.)_
...
registration (`qb/cmake/qbFunctions.cmake:982-989`) refuses to configure a module laid out any other
way.
