...
Read it as the platform's contract: on Windows `setLatency(1us)` and `setLatency(1ms)` sleep the same ~1.4–1.8 ms and the tick the folklore names (15.6 ms) is nowhere in it — the kernel's waits are tickless, its coalescing is the +0.3–0.8 ms; on Linux the value is honoured to the timer slack on both parks (the loop park since qb 3.2's `epoll_pwait2` wait; before it, and on a kernel older than 5.11, that park reads ~1 ms at 100 µs like Windows). Pick `latency` from the tail-latency budget, not from a number below what the platform can deliver. This drops idle CPU sharply, at the cost of a worst-case event-pickup delay bounded by `latency`. A producer that exhausts its send-retry budget wakes the destination's mailbox (`VirtualCore.cpp`), so a parked consumer is not left waiting the full interval under back-pressure.
...
3. **The actor object is arena-allocated; keep it under 1 KiB and 16-byte aligned to stay on that path.** `qb::Actor`'s `operator new` / `operator delete` draw from the core thread's `qb::allocator::thread_arena` (`qb/src/qb/core/Actor.h:534-555`, `qb/src/qb/system/allocator/thread_arena.h:93-103`): 16-byte size classes up to 1 KiB with LIFO reuse, chunks from `slab_cache`, no lock and no thread-local guard on the path — measured on the actor lifetime of qb-vs-others' `fib` at 185.3 -> 127.2 ns (-31 %) on Windows/MSVC and 129.2 -> 99.1 ns (-23 %) on WSL2 g++-14 (1c-spin). A type above 1 KiB or with `alignas` above 16 goes to the global allocator, so keep large state behind a pointer member. A derived class that declares its own `operator new` / `operator delete` replaces the arena for that type (a custom pool); `qb::allocate_actor<T>` remains the construction hook.
