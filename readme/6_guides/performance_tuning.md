...
- **`latency > 0`** — when idle, the core keeps polling for `setIdleSpin` (default 50 µs, `CoreInitializer::kDefaultIdleSpin`) measured from its first idle pass, and only then parks on a `std::condition_variable` (`_cv.wait_for(lk, _latency, has_data)` in the mailbox `wait()`) for up to `latency`, or until a producer enqueues an event and calls `notify()`. The floor is time, not an event count, so a reply that lands microseconds after a request is picked up on the polling path rather than after an OS park and wake; `setIdleSpin(qb::duration::zero())` parks on the first idle pass. Mind that a parked core does not consult qb-io's timers: a `qb::io::async::callback` armed on it fires when the park times out, so `latency` bounds timer precision too. This drops idle CPU sharply, at the cost of a worst-case event-pickup delay bounded by `latency`. A producer that exhausts its send-retry budget wakes the destination's mailbox (`VirtualCore.cpp`), so a parked consumer is not left waiting the full interval under back-pressure.
