...
<!-- src: qb/src/qb/core/Actor.h:1050-1062; qb/src/qb/core/Pipe.h:118-128 -->
...
`send<E>(dest, args...)` is a fire-and-forget variant. It returns nothing and makes **no ordering guarantee** (even between two sends from the same source to the same destination). `E` **should be trivially destructible**, and *must* be when it derives from `qb::EventQOS0` — the one class of event the engine is allowed to **drop** when a peer's mailbox is full, discarding it without running its destructor, which is where a heap-backed member would leak. That case is a compile error at every enqueue sink; for a plain `qb::Event` subclass the framework disposes the payload exactly once on every delivery path, so a `std::vector` member is legitimate though still worth avoiding in a fire-and-forget message. Use `qb::string<N>` or plain data members where you can. Prefer `push` unless ordering genuinely does not matter for the message.

The trivially-destructible requirement is a usage contract documented on `Actor::send` (`qb/src/qb/core/Actor.h`). It is enforced at compile time only for events that derive from `qb::EventQOS0`, through a `static_assert` in `qb::detail::routing_safe_type_id` (`qb/src/qb/core/Event.h`) — the one function every enqueue sink calls, so all four spellings are covered. A plain `qb::Event` subclass with a non-trivial member still compiles, and is disposed correctly on every delivery path; the constraint that remains yours to honor is trivial *relocatability*.
