...
```cpp
// src: qb/src/qb/io/async/coroutine/utils.h (doc example)
#include <qb/io/async.h>
#include <chrono>
...
- **`run_once()` can block on idle `ev_io`-only loops when timerfd is enabled.** qb's bundled libev disables timerfd by default (the `QB_EV_USE_TIMERFD` CMake option defaults to `OFF` in `qb/src/qb/ev/CMakeLists.txt`). If you build with `-DQB_EV_USE_TIMERFD=ON`, then when there are only `ev_io` watchers and no heap timers, `EVRUN_ONCE` can block for libev's internal maximum wait time. In pump loops prefer `run_until(status)` or `run(EVRUN_NOWAIT)` regardless.
