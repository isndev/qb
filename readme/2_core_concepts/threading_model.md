...
> **Audience:** Adopter · **Status:** stable · **Verified-against:** qb 3.1.0 (C++20 default, C++23 supported) @ 4816de9
...
`CoreInitializer::setAffinity(CoreIdSet const &)` requests that the `VirtualCore` thread be pinned to a set of physical CPU cores (`src/qb/core/Main.h:275`). The pin is applied when the engine starts, inside `VirtualCore::__init__` (`src/qb/core/VirtualCore.cpp:498`), using `pthread_setaffinity_np` on POSIX/macOS and `SetThreadAffinityMask` on MSVC Windows.
...
Each `VirtualCore` consumes from exactly one inbound mailbox. A `Mailbox` is a multi-producer/single-consumer lock-free ring buffer of `EventBucket` slots (`src/qb/core/Main.h:438`), built on `qb::lockfree::mpsc::ringbuffer` (`src/qb/system/lockfree/mpsc.h:47-48`). All mailboxes are owned by an internal `SharedCoreCommunication` instance held by `qb::Main` (`src/qb/core/Main.h:391`). The MPSC shape is the crux of the threading model: many sender cores enqueue into a mailbox, but only the owning `VirtualCore` ever dequeues from it.
...
- `setLatency(d)` with `d > 0` lets the core park for up to `d` when idle (less when a timed request's deadline is nearer: the deadline bounds the park) — inside its io loop when it owns active qb-io watchers, so io readiness and io timers still wake it at poll latency, on a `std::condition_variable` when it owns none (`src/qb/core/Main.h:285-300`, mailbox `wait()` at `src/qb/core/Main.h:502-512`, `wait(listener &)` at `src/qb/core/Main.h:553-568`). A peer enqueuing an event calls `notify()` to wake it either way. This trades worst-case wake-up latency for lower CPU — and the wait is only as fine as the platform's: an idle core asked for 100 µs wakes every ~1.4–1.5 ms on Windows through either park (both waits take whole milliseconds there, and the kernel coalesces on top; `setLatency(1us)` and `setLatency(1ms)` sleep the same) and every ~163 µs on Linux 5.11 or newer (the 50 µs timer slack plus a pass, the loop park through `epoll_pwait2`). The figures and the probe are in the [performance tuning guide](../6_guides/performance_tuning.md#event-loop-idle-latency).
...
The loop does not park the instant it goes idle. The first pass that moved no event and left nothing in the core's own pipe stamps `_idle_since`; the core keeps polling the lock-free fast path until `setIdleSpin` has elapsed since that stamp, and only then parks — `_mail_box.wait(loop)` inside its io loop when that loop has work, `_mail_box.wait()` on the mailbox condition variable otherwise (`src/qb/core/VirtualCore.cpp:929-956`). The floor is time, not an event count: a request/response exchange whose reply lands a few hundred nanoseconds after the request left stays on the polling path instead of paying an OS park and wake on every hop, and a genuinely idle core still parks within ~50 µs. When latency is zero the core never parks, but its idle pass still takes the same monotonic clock read: measured on i9-12900K, that ~13 ns of serialized work is what paces the idle poll of the peer rings, and a tighter idle pass made the cross-core spin exchange 25 % slower (the loop-clock pass of the qb-vs-others audit). The park itself is race-free: `wait()` publishes a `_parked` flag, fences, re-checks the ring and waits on a predicate, while `notify()` fences after the enqueue and takes the mutex before it signals — the two fences forbid the enqueue-between-drain-and-wait window that the 3.0 form slept through (`src/qb/core/Main.h:502-512`, `src/qb/core/Main.h:611-645`). [The engine](../4_qb_core/engine.md) has the measured account.

Two scopes set latency:

| Call | Scope | Signature |
| --- | --- | --- |
| [`CoreInitializer::setLatency`](../4_qb_core/engine.md) | One core | `CoreInitializer &setLatency(qb::duration latency = qb::duration::zero()) noexcept` (`src/qb/core/Main.h:300`) |
| [`Main::setLatency`](../4_qb_core/engine.md) | Every registered core | `void setLatency(qb::duration latency = qb::duration::zero())` (`src/qb/core/Main.h:968`) |
| [`CoreInitializer::setIdleSpin`](../4_qb_core/engine.md) | One core | `CoreInitializer &setIdleSpin(qb::duration idle_spin = kDefaultIdleSpin) noexcept` (`src/qb/core/Main.h:317`) |
| [`Main::setIdleSpin`](../4_qb_core/engine.md) | Every registered core | `void setIdleSpin(qb::duration idle_spin = CoreInitializer::kDefaultIdleSpin)` (`src/qb/core/Main.h:977`) |

`Main::getLatency()` does not exist; read a single core's configured value with `engine.core(id).getLatency()`, which returns the `qb::duration` last set (`src/qb/core/Main.h:350`); `getIdleSpin()` is its twin (`src/qb/core/Main.h:355`).
...
All cores and their per-core settings must be configured *before* `Main::start()`. Calling `engine.core(id)` after the engine is running throws `std::runtime_error("Cannot access to CoreInitializers while engine is running")` (`src/qb/core/Main.cpp:607-609`). A core registered with zero actors fails engine startup, so only call `core(id)` for cores that will receive at least one actor (`src/qb/core/Main.cpp:425-427`).
...
A per-core `setLatency` is idempotent — calling it more than once on the same core overwrites the previous value (`src/qb/core/Main.cpp:74-77`). `Main::setLatency` applies unconditionally to all registered cores (`src/qb/core/Main.cpp:499-502`), so a per-core override must follow it, not precede it. `setIdleSpin` follows both rules (`src/qb/core/Main.cpp:80-83`, `src/qb/core/Main.cpp:505-508`).
...
- **Configuration after `start()` throws.** Affinity, latency, and actor registration are start-time only. `engine.core(id)` after start throws `std::runtime_error` (`src/qb/core/Main.cpp:607-609`); read-only `usedCoreSet()` remains safe.
- **Empty cores abort startup.** Registering a core with no actors logs `VirtualCore(<id>).id(<thread>) Started with 0 Actor` and fails the start (`src/qb/core/Main.cpp:425-427`). Only configure cores you will populate.
- **`async = false` consumes the calling thread.** `start(false)` runs the last core's event loop on the caller (`src/qb/core/Main.cpp:539-544`) and blocks until shutdown, so `join()` is neither needed nor reached afterward. Use the default `start()` plus `join()` when the calling thread must keep doing other work.
