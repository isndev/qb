...
- `setLatency(d)` with `d > 0` lets the core park for up to `d` when idle (less when a timed request's deadline is nearer: the deadline bounds the park) — inside its io loop when it owns active qb-io watchers, so io readiness and io timers still wake it at poll latency, on a `std::condition_variable` when it owns none (`src/qb/core/Main.h:284-290`, mailbox `wait()` at `src/qb/core/Main.h:468-478`, `wait(listener &)` at `src/qb/core/Main.h:519-534`). A peer enqueuing an event calls `notify()` to wake it either way. This trades worst-case wake-up latency for lower CPU.
...
| [`Main::setLatency`](../4_qb_core/engine.md) | Every registered core | `void setLatency(qb::duration latency = qb::duration::zero())` (`src/qb/core/Main.h:933`) |
...
| [`Main::setIdleSpin`](../4_qb_core/engine.md) | Every registered core | `void setIdleSpin(qb::duration idle_spin = CoreInitializer::kDefaultIdleSpin)` (`src/qb/core/Main.h:942`) |
