...
- `setLatency(d)` with `d > 0` lets the core park for up to `d` when idle (less when a timed request's deadline is nearer: the deadline bounds the park) — inside its io loop when it owns active qb-io watchers, so io readiness and io timers still wake it at poll latency, on a `std::condition_variable` when it owns none (`src/qb/core/Main.h:285-291`, mailbox `wait()` at `src/qb/core/Main.h:502-512`, `wait(listener &)` at `src/qb/core/Main.h:553-568`). A peer enqueuing an event calls `notify()` to wake it either way. This trades worst-case wake-up latency for lower CPU.
