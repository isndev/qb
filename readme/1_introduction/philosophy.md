...
- **Ordered and unordered delivery.** `push<Event>()` delivers in FIFO order relative to other pushes from the same source to the same destination (`src/qb/core/Pipe.h:131-132`); `send<Event>()` is unordered, fire-and-forget delivery, conventionally reserved for trivially-destructible events. Choosing the weaker guarantee where ordering is not needed avoids paying for it.
