...
> **Audience:** Evaluator · **Status:** stable · **Verified-against:** qb 3.1.0 (C++20 default, C++23 supported)
...
- **Ordered and unordered delivery.** `push<Event>()` delivers in FIFO order relative to other pushes from the same source to the same destination (`src/qb/core/Pipe.h:128-129`); `send<Event>()` is unordered, fire-and-forget delivery, conventionally reserved for trivially-destructible events. Choosing the weaker guarantee where ordering is not needed avoids paying for it.
