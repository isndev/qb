...
> <!-- src: qb/src/qb/core/Actor.h:902-911 -->
...
> <!-- src: qb/src/qb/core/Actor.h:912-915 -->
...
> (`qb/src/qb/core/Actor.h:691-694`), so on a destroyed actor the guard *is* the use-after-free.
>
> `spawn` registers the coroutine in the actor's cancellation scope
> (`qb/src/qb/core/Actor.h:1308-1309`), and `Actor::kill()` cancels that scope
> (`qb/src/qb/core/Actor.cpp:399-410`), so the wait simply stops existing. Note that this is not a
