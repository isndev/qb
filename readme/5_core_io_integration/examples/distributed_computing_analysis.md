...
This example measured it: three runs of three aborted under the `sanitize` preset with `AddressSanitizer: heap-use-after-free`, reported inside such a lambda at `WorkerNodeActor::scheduleMetricsUpdate()` twice and `TaskGeneratorActor::scheduleTaskGeneration()` once. All eight sites in this file are now `spawn(...)` + `co_await ctx.sleep(d)`, as are 30 across the four programs that were swept — 8 here, 17 in `example8_state_machine.cpp`, 4 in `example9_trading_system.cpp`, 1 in `qbm/http/02_simple_client.cpp`.

**The sweep did not reach the whole corpus, and the remainder is not a rounding error.** Counted over `examples/` today, **21** `this`-capturing *timed* `qb::io::async::callback(f, delay)` call sites survive in programs the two repair commits did not touch: `qbm/redis` (10 — examples 4, 5, 6 and 7), `core_io/file_monitor` (3), `core_io/chat_tcp` (2), `core_io/file_processor` (2), `core_io/message_broker` (2), `qbm/ws/02_chat_client.cpp` (2). A further 5 sites capture `this` but use the **no-delay** overload, which runs inline and is therefore not this hazard at all. Each case-study page for those programs now flags the shape at the call site rather than presenting it as an idiom — but the code still has it, so read those examples as material to correct, not to copy.
