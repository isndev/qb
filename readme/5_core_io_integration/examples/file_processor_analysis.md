...
- The type is **`qb::io::sys::file`** (namespace `qb::io::sys`), included from `<qb/io/system/file.h>`. There is no `qb::io::system::file` alias — some of the example's own doc comments use that spelling, and they are wrong. <!-- src: qb/src/qb/io/system/file.h:44,56 -->
