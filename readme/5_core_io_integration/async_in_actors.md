...
VirtualCore::__workflow__                          ← pass N
 ├─ listener::current.run(EVRUN_NOWAIT)            step 3 (VirtualCore.cpp:814)
...
Both return immediately and **share the same safety contract**, because a coroutine frame can still be running a step while — or just after — its actor is destroyed: <!-- src: qb/src/qb/core/Actor.h:1347-1363,1394,1431 -->
...
- **After suspension, use only the context.** `ctx.push<Event>(...)` (to the spawning actor), `ctx.push_to<Event>(dest, ...)`, `ctx.broadcast<Event>(...)`, `ctx.id()` and `ctx.time()` are safe; an event addressed to an actor that is gone finds no handler and is disposed. <!-- src: qb/src/qb/core/VirtualCore.h:1305-1323 -->
...
```
<!-- src: qb/src/qb/core/Actor.h:1430-1431,2166-2168, qb/src/qb/core/Actor.cpp:555-566 -->
