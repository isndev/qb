...
<!-- src: qb/cmake/qbFunctions.cmake:485-492, qb/tests/io/unit/CMakeLists.txt:27 -->
...
When `QB_BUILD_TESTS` is `OFF`, `qb_add_test` returns early before defining any target, so no test executables and no CTest registrations exist (`qb/cmake/qbFunctions.cmake:494-497`).

Test executables are written to `${CMAKE_BINARY_DIR}/bin/tests` (`qb/cmake/qbFunctions.cmake:561-563`), not alongside the per-module build trees. After a build, list them with:
