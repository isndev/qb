...
> **Audience:** Contributor · **Status:** stable · **Verified-against:** qb 3.2.0 (C++20 default, C++23 supported)
...
To build and run the suite instrumented, configure with a sanitizer preset rather than a plain Debug build: `cmake --preset sanitize` for AddressSanitizer + UndefinedBehaviorSanitizer, or `cmake --preset sanitize-thread` for ThreadSanitizer. The `QB_SANITIZE` flag (default empty) applies its sanitizer list to every qb, qbm, and test target plus the link step, regardless of `CMAKE_BUILD_TYPE`. The GitHub Actions `sanitize` and `sanitize-thread` workflows run these presets on Ubuntu with Clang. A sanitized configure never takes a system GoogleTest: the framework is built from the pinned tag so that it is instrumented like the code under test — a non-instrumented one produces `container-overflow` false positives at the library boundary (`qbFetchGoogleDeps.cmake:53-61`). See `QB_SANITIZE` in [CMake options](./cmake_options.md).
...
A system test drives the actor runtime. The common pattern is `start()` then `join()`: `Main::start(bool async = true)` defaults to `async = true`, spawning worker threads and returning immediately, after which `join()` blocks until every core has stopped (`qb/src/qb/core/Main.h:894,919`). Passing `start(false)` instead turns the calling thread into a worker and blocks inline until the engine stops (`qb/src/qb/core/Main.h:894`, `qb/src/qb/core/Main.cpp:538-543`). Either way, collect results into an `std::atomic` (or a response event) and assert after the run completes, including on `Main::hasError()`.
