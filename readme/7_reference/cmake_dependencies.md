...
|---|---|---|---|---|
| libev | qb fork, vendored (`include/qb/vendor/qev`) | required | event loop (core/io) | `qbDependencies.cmake` |
...
- **libev** — REQUIRED. `qbDependencies.cmake:104-116` checks that `include/qb/vendor/qev` exists and sets `QB_HAS_LIBEV`; if it is missing, configuration fails with a fatal error. The tree is compiled by `add_subdirectory("${QB_VENDOR_DIR}/qev")` (`qb/CMakeLists.txt:100`), producing the static `qev` target (`include/qb/vendor/qev/CMakeLists.txt:287`). Resolving libev defines `QB_HAS_LIBEV=1` on every qb target (`qbDependencies.cmake:390-392`).
