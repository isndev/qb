...
- **nanolog, ska_hash** — header-only, no CMake target at all. They are ordinary files under `src/qb/vendor/`, reached through qb's single include root like any other qb header. `source/io/src/io.cpp` compiles `nanolog.cpp` by textual inclusion.
