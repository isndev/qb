...
| `qb::allocator::thread_arena` | `qb/system/allocator/thread_arena.h` | Per-thread size-class arena (16-byte classes up to 1 KiB, LIFO reuse, chunks from `slab_cache`) that `qb::Actor::operator new` / `operator delete` draw from; a block is freed on the thread that allocated it, and a thread that exits with a live block orphans its chunks (kept, counted, never recycled). |
