...
- `Actor::kill()` sets `_alive = false` and calls `VirtualCore::killActor(id())` (`src/qb/core/Actor.cpp:398-406`). It only *flags* the actor: the actor stops receiving new events but may still drain events already queued, and `~Actor()` runs later under `VirtualCore` control, at the end of the workflow iteration.
...
- Service-actor ids are reserved for the lifetime of the process. `VirtualCore::removeActor` recycles only non-service ids back into the pool, keeping each `ServiceIndex` stable (`src/qb/core/VirtualCore.cpp:1036-1040`).
...
- `reply(event)` returns a received event to its source by swapping `dest` and `source` (`src/qb/core/Actor.cpp:416-423`). `forward(dest, event)` re-routes it to a new destination while **preserving the original source** (`src/qb/core/Actor.cpp:425-434`).
...
- Broadcast events cannot be replied to or forwarded; the attempt is logged and dropped (`src/qb/core/Actor.cpp:418-421`, `src/qb/core/Actor.cpp:429-432`).
...
- Shutdown has three triggers wired to the same plumbing (`src/qb/core/VirtualCore.cpp:736-761`; `Main::stop()` at `src/qb/core/Main.cpp:565-572`): a POSIX signal (`SIGINT` / `SIGTERM` via `sigaction`), `Main::stop()` setting a `std::atomic<std::sig_atomic_t>` that the workflow polls, and the C++20 `qb::stop_source` (`request_stop()` on `~Main` or programmatically). A worker that observes any of them synthesizes a virtual `SIGINT` and broadcasts a `SignalEvent`, so existing shutdown handlers keep working on platforms with or without POSIX signals.
