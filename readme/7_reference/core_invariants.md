...
- Default event registrations (`KillEvent`, `SignalEvent`, `UnregisterCallbackEvent`, `PingEvent`, `RequireEvent`) can be opted out by passing `qb::no_default_events` to the protected constructor (`src/qb/core/Actor.h:305`). Use this for ephemeral, pool-reused actors that never receive those control events.
...
- Each event type has a stable 16-bit `qb::TypeId` assigned at first use through a magic-static, atomically incremented counter (`qb::type_id<T>()`, `src/qb/core/Event.h:62`). `ServiceActor<Tag>` indices are allocated once under that same magic-static guarantee, with a mutex guarding the shared service-id map insertion, so the id is unique and valid even under concurrent first use (`src/qb/core/VirtualCore.h:1036-1046`).
...
- `ActorId::BroadcastSid` (`ServiceId` max) marks an id as a broadcast target. A `qb::BroadcastId(core)` used with `push<>()` delivers to every actor on one core; `Actor::broadcast<Event>()` fans out to every active core (`src/qb/core/Actor.h:1001-1008`).
...
- `push<Event>()` guarantees ordered delivery to the same destination from the same source (`src/qb/core/Actor.h:875-876`, mirrored by `qb::Pipe::push`, `src/qb/core/Pipe.h:127-130`). It returns a mutable reference to the event in the pipe buffer; you may set fields on it before it is consumed, but you must not store the reference past the current scope.
...
- Two entry points run a C++20 coroutine inside an actor: `Actor::spawn(func)` (recommended — *scoped*, detailed below) and `Actor::spawn_detached(func)` (*detached*) (`src/qb/core/Actor.h:1238-1239` and `:1150`). Both must be called from the actor's worker thread; a debug assertion checks that a thread-local coroutine scheduler exists on the calling thread (`src/qb/core/Actor.cpp:260-262`).
...
- The active-coroutine counter is an eagerly allocated `shared_ptr<atomic<size_t>>` (`src/qb/core/Actor.h:1344`), so each `spawn`/`spawn_detached` is a single `fetch_add` plus a spawn on the hot path. Query it with `has_active_coroutines()` (`src/qb/core/Actor.h:1212`) or `active_coroutine_count()` (`src/qb/core/Actor.h:1248`).
...
- Shutdown has three triggers wired to the same plumbing (`src/qb/core/Main.cpp:455-462`): a POSIX signal (`SIGINT` / `SIGTERM` via `sigaction`), `Main::stop()` setting a `std::atomic<std::sig_atomic_t>` that the workflow polls, and the C++20 `qb::stop_source` (`request_stop()` on `~Main` or programmatically). A worker that observes any of them synthesizes a virtual `SIGINT` and broadcasts a `SignalEvent`, so existing shutdown handlers keep working on platforms with or without POSIX signals.
...
| `VirtualCore` actor maps / callback list | plain read/write | mutated only on the owning worker thread (`src/qb/core/VirtualCore.h:172`) |
