...
A parameter typed `qb::duration` — `qb::CoreInitializer::setLatency(qb::duration latency = qb::duration::zero())` is the canonical one (`src/qb/core/Main.h:295`) — accepts any `std::chrono` literal whose period is finer than or equal to a nanosecond, implicitly:
