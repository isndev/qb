...
- `push` calls `allocate_back` (`src/qb/core/VirtualCore.h:1080`). The event joins the FIFO stream at the tail and is delivered in the next flush, **in order** with everything already queued to that core.
...
The consumer that explained why it existed was `VirtualCore::__receive__`, which used to swap the same-core pipe out before draining it. That pipe is a `segmented_pipe<EventBucket>` now (`VirtualPipe`, `src/qb/core/Event.h:869`), whose own `swap` exchanges six members and needs no byte trick because a segmented pipe is not one cache line (`src/qb/system/allocator/segmented_pipe.h:687-694`) — and since 3.2 the drain does not swap at all. It walks the one self pipe **in place, up to a fence** taken at the top of the pass (`segmented_pipe::mark()`, `front(fence &)`, `pop_front(fence &)`, `src/qb/system/allocator/segmented_pipe.h:604-645`):
...
```
<!-- src: qb/src/qb/core/VirtualCore.cpp:260-266 -->
...
Two rules make the arena lock-free and guard-free, and both are the engine's own invariants rather than new ones. A block is freed on the thread that allocated it — an actor is constructed on its core's thread and destroyed there, in the reap phase — so there is no cross-thread free path at all. And the thread-local state is a constant-initialised aggregate, so `allocate` / `deallocate` are a plain TLS load away, with the cleanup in a separate function-local thread-local that runs at thread exit (`src/qb/system/allocator/thread_arena.h:243-277`), the shape the coroutine frame pool uses. At that exit the slabs go back to the cache only if nothing is live; a thread that ends with a live block — a contract violation — keeps its chunks on a process-wide orphan list, reachable and counted by `thread_arena::orphaned()`, never recycled (`src/qb/system/allocator/thread_arena.h:308-351`): a bounded, visible retention is the safe failure, a use-after-free through a recycled slab is not. Under AddressSanitizer the arena poisons every block it holds -- a freed block with its link word, a chunk's unreserved tail -- and unpoisons a block as it hands it out, so a read through a dead actor's pointer is reported as `use-after-poison` with the access stack, the report `malloc` gave and a recycling arena would otherwise take away; no other build carries a byte of it.
...
| per-core receive buffer | `N` | 65 536 B | one `_event_buffer` per core — a `std::array<EventBucket, 1024>` behind a `unique_ptr` (`src/qb/core/VirtualCore.h:384`) |
...
The arithmetic was checked against the process on 2026-09-09 with qb-vs-others' `pipe-footprint` probe (one actor per core, the engine settled at `latency` 100 µs, the resident set read from the main thread; `results/<host>/qb-63-footprint/` there, TUNING §20): resident growth over the process's baseline, Windows 11 / MSVC 19.51 and WSL2 Linux 6.6 / g++ 14 within 5 % of each other —

| Cores | at rest, measured | after one event on every pipe (`N × (N − 1)` segments) | after the engine is destroyed |
|---|---|---|---|
| 8 | 4.1–5.1 MiB | 20–21 MiB | the rings freed; 17 MiB of slabs stay in the cache |
| 32 | 65–68 MiB | 324 MiB (Linux) / 325 MiB committed, 72 MiB resident (Windows) | idem, 257 MiB cached |
| 64 | 263–266 MiB | 1.26 GiB (Linux) / 1.26 GiB committed, 282 MiB resident (Windows) | idem, 1.0 GiB cached |
| 128 | 1.02 GiB | 5.02 GiB (Linux) / 5.04 GiB committed, 1.09 GiB resident (Windows) | idem, 4.0 GiB cached |

Three readings. The resting cost is the **rings**, `N² × 64 KiB`, and it is resident from `Main::start()` because the mailbox value-initialises them (a 256-core engine would sit at ~4 GiB before its first event — the arithmetic, not a measurement). The traffic cost is **256 KiB per pipe that ever carried an event**, up to `N × (N − 1)` on a fan-out topology; on Linux a slab is populated when it is mapped (the huge-page prefault above), so that memory is resident the moment the pipe grows, on Windows it is committed and becomes resident page by page as events land. And what an engine leaves behind when its cores stop is the **slab cache**: the rings are freed, the slabs go to the process-wide free list, mapped and warm for the next engine — `slab_cache::trim()` is the call that returns them to the OS, and nothing calls it for you. The shape is what matters: **doubling the core count roughly quadruples the resting footprint**, because every mailbox keeps a private inbound ring from every other core — and, until 3.2, every core kept an eagerly allocated private outbound pipe to every other core, which was the larger of the two terms. The pipes now cost what they hold: a core's pool grows to the high water of the busiest burst its pipes have seen, in 2 MB slabs, and stays there — 2 MB per core that has ever pushed is the floor, and a 16-core engine at rest after a light exchange is 32 MB of slabs rather than the 68 MiB of eager pipes it used to be. That is the price of a message path with no lock and no shared cursor on it, and it is worth knowing before you configure a 64-core engine on a container with a memory limit.
