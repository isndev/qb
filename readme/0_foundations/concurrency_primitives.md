...
A bounded, wait-free single-producer/single-consumer FIFO. The producer advances `write_index_`; the consumer advances `read_index_`. Each side owns two cache lines, each in its own two-line block (a line and its spatial-prefetch partner): a *private* line holding its working index and a snapshot of the *other* side's index (`write_index_local_` + `cached_read_index_` for the producer, `read_index_local_` + `cached_write_index_` for the consumer), and a *published* line holding nothing but the index the peer reads (`src/qb/system/lockfree/spsc.h:95-114`). The fast path proves "not full" / "not empty" from its private line alone and re-reads the peer's published line only when the snapshot says otherwise; and a side only ever *writes* the line it publishes on — the working copy is what it reads. That last rule is measured, not stylistic: until 3.2 `write_index_` shared its line with the producer's snapshot, so every `enqueue` re-read the line the consumer was polling, a cross-core miss on every hop once the poll had snooped it away; keeping the working index private took qb's two-core ping-pong from 212 to 165 ns per round trip and the 100-actor cross-core ring from 115 to 80 ns per hop, same-core figures unchanged. Coordination uses acquire/release ordering on the published indices; no lock is taken on either side.
...
- **`consume_all` splits an item that spans the wrap.** Use the copy-out `dequeue(func, scratch, n)` overload whenever one logical item can occupy more than one slot (`src/qb/system/lockfree/spsc.h:390-398`).
