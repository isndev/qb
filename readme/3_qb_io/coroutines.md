...
<!-- src: qb/src/qb/io/async/listener.h:1354 (init), qb/src/qb/io/async/coroutine/utils.h:212 (coro_scheduler), :227 (run_for), :285 (run_sync), examples/03-coroutines/01-first-coroutine.cpp:144-175 -->
...
<!-- src: qb/src/qb/io/async/coroutine/scheduler.h:825 (active_count), :660-669 (re-entrancy guard), :756 (is_draining_ready); qb/src/qb/io/async/listener.h:1369 (ensure_not_inside_ready_drain) -->
...
| Rule | Reason | Source |
|---|---|---|
| Event handlers stay `void on(Event&)` | `registerEvent` requires a `void` handler; a `task<void> on(Event&)` breaks actor dispatch | `Actor.h:960` |
| Use `spawn()` (or `spawn_detached()`) for coroutine work | isolates the coroutine from live actor state | `Actor.h:1431`, `:1394` |
| Capture by **value** inside the lambda | a reference (or `this`) dangles after the first `co_await` | `Actor.h:1353-1355`, `:1411-1412`; examples/03-coroutines/02-actor-coroutines.cpp:138 |
| Communicate via `ctx.push` / `ctx.push_to` | preserves message-passing semantics; an event addressed to an actor that is already gone finds no subscribed handler, so it is disposed instead of delivered | `Actor.h:1610-1611` (`push`), `:1622-1623` (`push_to`); `qb/src/qb/system/event/router.h:493-501` (no handler → dispose, no dispatch) |
| Process results in a synchronous handler | guarantees exclusive access to actor state | `Actor.h:1349-1351` |
...
A coroutine parked on a cancellation-aware operation unwinds promptly, because that awaiter registered a hook. All four of the context's own operations qualify: `ctx.sleep(d)` is `cancellable_sleep` (`src/qb/core/Actor.h:2168`), `ctx.until_cancelled()` is `check_cancelled` (`src/qb/core/Actor.h:2189`), `ctx.cancellable(t)` is `make_cancellable` (`src/qb/core/Actor.h:2201`), and `qb::ask` links an embedded `cancel_hook` on the same token — no `std::function`, nothing allocated, unlinked in O(1) when the reply lands (`src/qb/core/Actor.h:1895`). `ctx.cancellation_point()` is a near relative rather than a member of that set: it returns a `yield_or_cancel` that hands the loop a turn and throws if the token fired while it was away (`src/qb/core/Actor.h:2179`), so it is prompt inside a loop but cannot be woken out of a long wait.
...
<!-- src: qb/src/qb/core/Actor.cpp:532; qb/src/qb/io/async/listener.h:1369 (ensure_not_inside_ready_drain) -->
