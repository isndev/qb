...
It also shows why the `size <= pendingRead()` guard cannot be unconditional: `transport::accept` has no `in()` and no `pendingRead()` at all, so the check is compiled out for it by a `requires` test (`src/qb/io/async/io.h:2600-2606`) rather than failing every accept.
...
- **Size limits are enforced, not advisory — and the two limits are different codes.** A `getMessageSize()` result above `max_message_size()` (default `QB_MAX_MESSAGE_SIZE`) marks the protocol `not_ok()` and disconnects with `message_too_large`, reason **`-2`** (`src/qb/io/async/io.h:2608-2613`). A read or write buffer that would exceed `max_read_buffer_size()` / `max_write_buffer_size()` (default `QB_MAX_WRITE_BUFFER_SIZE`, `QB_MAX_READ_BUFFER_SIZE`) disconnects with `buffer_overflow`, reason **`-3`**, and does *not* touch the protocol's `ok()` at all (`src/qb/io/async/io.h:2776-2779`). Raise a component's limit deliberately with `set_max_message_size()` / `set_max_read_buffer_size()` rather than assuming it is unlimited.
