...
> **Audience:** Evaluator · **Status:** stable · **Verified-against:** qb 3.0.1 (C++20 default, C++23 supported) — f1d8cca6
...
- **Deferred continuation (`qb::io::async::defer`)** — queue a callable to run once at the tail of the current loop turn, after every watcher has returned. The one correct primitive for continuing after a handler unwinds — above all when the handler must destroy or replace the object it is running on.
- **Delayed callbacks (`qb::io::async::callback`, `scoped_callback`)** — schedule a callable to run after a `qb::duration` delay. `callback` is fire-and-forget; `scoped_callback` returns an RAII handle that cancels the timer on destruction. **Note the name:** `callback(f)` with no delay (and `callback(f, d)` with `d <= 0`) runs `f` **inline and immediately** — it does not defer. Use `defer` for that.
