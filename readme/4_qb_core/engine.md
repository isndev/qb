...
_metrics.reset();
```
<!-- src: qb/src/qb/core/VirtualCore.cpp:885-903 -->
...
The `onInit()`-throws case is worth stating exactly, because it is easy to guess wrong. `__drive_init__` is `noexcept` and catches the exception itself, logs it, and reports `InitOutcome::ReadyFalse` (`src/qb/core/VirtualCore.cpp:579-591`). It therefore lands on `BadActorInit`, identically to a clean `co_return false`. `ExceptionThrown` is reserved for a throw that actually escapes — a throwing constructor during `create()`, or a handler throwing inside `__workflow__`.
...
One iteration of `__workflow__`, in the order it happens (`src/qb/core/VirtualCore.cpp:721-912`):
...
- **`Actor::time()` is constant for a whole pass, and sampled on demand.** It forwards to `VirtualCore::time()`, which reads `qb::unix_nanos(qb::wall_now())` on the FIRST call of a pass — keyed on `_loop_count` — and returns that sample to every later call in the same pass (`src/qb/core/VirtualCore.cpp:1135-1142`; `src/qb/core/VirtualCore.h:412-414`). Every actor on the core sees the same value in the same pass, and so does `qb::LoopEvent::now`; a pass in which nobody asks reads no clock at all (until 3.0 every pass paid one read at step 1, asked or not). For a moving clock read `qb::wall_now()` directly; `Actor::now()` is the same cached instant as a `qb::wall_time`.
- **A `qb::ICallback` tick happens *after* the flush**, so events a tick pushes wait for the *next* pass to leave the core. Events pushed from an ordinary handler in step 6 also miss step 5, for the same reason. One `qb::LoopEvent` is built for the whole pass and handed to every registered callback by a direct virtual call — it is not a routed event and has no source or destination (`src/qb/core/VirtualCore.cpp:799-803`; `src/qb/core/ICallback.h:65-80`).
