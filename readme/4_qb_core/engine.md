...
> **Audience:** Adopter · **Status:** stable · **Verified-against:** qb 3.1.0 (C++20 default, C++23 supported) — d519f005
...
Between them sits **`SharedCoreCommunication`**, which `Main` owns and no application code names. It holds the `CoreSet`, one `Mailbox` per used core, and one atomic "has stopped" flag per core (`src/qb/core/Main.h:670-678`). A `Mailbox` is a `qb::lockfree::mpsc::ringbuffer<EventBucket, MaxRingEvents, 0>` (`src/qb/system/lockfree/mpsc.h`) plus the condition variable used by the latency path. The matching send side lives inside each `VirtualCore`: one `qb::VirtualPipe` per destination core, plus one for itself. [Inter-actor messaging](./messaging.md#the-address-is-the-route) narrates a single event through both.

A *core index* is a logical `qb::CoreId`, not a CPU. The valid range is `0 … qb::MaxCores - 1` (256); `core(index)` with a larger index throws `std::range_error`, and the `qb::NoAffinity` sentinel is `CoreId` max, deliberately above `MaxCores`, so it can never be mistaken for one (`src/qb/core/Main.cpp:617-618`; `src/qb/core/Main.h:107`).

`qb::Main()` registers **no** cores. A core comes into existence the first time you call `core(index)` or `addActor(index, …)` for it; there is no default population from hardware concurrency (`src/qb/core/Main.cpp:606-620`).
...
`CoreInitializer::builder()` returns an `ActorBuilder` whose `addActor` chains; `idList()` gives the created ids in creation order as a `qb::Main::ActorIdList` (a `std::vector<qb::ActorId>`), and `valid()` — also reachable through `explicit operator bool` — reports whether every add on that builder reserved an id (`src/qb/core/Main.h:131-198`; `src/qb/core/Main.cpp:120-135`).

**`addActor` does not construct the actor.** It reserves an `ActorId`, stores a `TActorFactory` holding your decayed constructor arguments, and returns the id immediately (`src/qb/core/Main.h:1072-1097`). The object is built much later, on the worker thread, when `start()` calls `factory->create()` (`src/qb/core/Main.cpp:428-429`). Two consequences you can see from the call site:

- **String literals become `std::string`** and every other argument is decayed and stored by value, because the factory has to outlive the call. Anything matching the `reference_wrapper_type` concept is preserved as-is (`src/qb/core/Actor.h:2483-2517`, `:2542-2551`).
- **A valid id is not a promise that the actor will exist.** `qb::ActorId::NotFound` (`0`, the default-constructed id — test it with `is_valid()`) comes back only when the *reservation* fails: a second `ServiceActor` of a type already on that core, or `_next_id` reaching `ServiceId` max. An `onInit()` that later fails is reported through `hasError()`, never by changing the id you already have (`src/qb/core/Main.h:1076-1094`; `src/qb/core/ActorId.h:470-471`).

The reservation and the real allocation are two different counters, and three things make them line up. `CoreInitializer::_next_id` starts at `_nb_service + 1` (`src/qb/core/Main.cpp:44`) and hands out `ActorId(_next_id++, index)`; on the worker, `Actor`'s constructor calls `VirtualCore::__generate_id__()`, which draws the *lowest free* slot from a per-core bitset pool seeded at exactly the same value (`src/qb/core/VirtualCore.cpp:116`, `:134-142`); and the worker constructs **every** registered actor before it runs a single `onInit()` (`src/qb/core/Main.cpp:428-432`), so nothing else can draw from that pool in between. Same seed, same order, one at a time — so the id you were handed is the id the actor gets. Service actors sidestep both: `ServiceActor<Tag>::ServiceIndex` is assigned once per tag at static-init time and is *below* the pool's floor, which is why a service id is stable, deterministic (the first one is `1`) and never recycled (`src/qb/core/VirtualCore.h:1289-1303`; `src/qb/core/VirtualCore.cpp:1121-1125`).
...
```
<!-- src: qb/src/qb/core/Main.cpp:608-609 -->
...
```
<!-- src: qb/src/qb/core/Main.h:502-512 -->
...
| `> 0` | parks for up to that span, once the core has been idle for its **idle-spin floor** (`setIdleSpin`, 50 µs by default) — inside its event loop when it owns active qb-io watchers (`ev_run(EVRUN_ONCE)` capped at `latency`), on a `std::condition_variable` when it owns none | lower CPU at idle; a core parked in its loop still answers io and io timers at poll latency, a core parked on its mailbox wakes only for a producer's `notify()` or the timeout |
...
A producer that enqueues into a mailbox calls `notify()` afterwards (`src/qb/core/Main.cpp:242`; the budget-exhausted flush at `src/qb/core/VirtualCore.cpp:476`). The two halves are a Dekker handshake, not a bare signal: `wait()` publishes a `_parked` flag, fences, re-checks the rings and only then sleeps — with `has_data()` as the predicate — while `notify()` fences, reads the flag, and takes the mutex before `notify_one()` when the consumer has announced a park (`src/qb/core/Main.h:502-512`; `src/qb/core/Main.h:611-645`). That is what makes a wake-up impossible to lose: the 3.0 form (`wait_for` with no predicate, `notify_all` outside the lock) slept the whole latency whenever an enqueue landed between the consumer's empty drain and its registration on the condition variable — measured on a two-core ping-pong, 246–293 lost wakeups per 300 000 messages on Linux and up to 2849 on Windows, where MSVC's `wait_for` rounds to the 15.6 ms scheduler tick. At zero latency `wait()` returns at once and `notify()` is only its fence — kept on purpose: on x86 a `seq_cst` fence drains the producer's store buffer, so the event just enqueued becomes visible to the spinning consumer now rather than whenever the buffer flushes. Measured on the same two-core ping-pong (interleaved A/B on a quiet host, p50 of 7 runs), the spinning cell was *slower* than the parking one without it on Windows — 296–309 ns per round-trip against 256–272 — and lands at 259–263 ns with it; on Linux the p50 is a wash (204–219 → 205–208) and the worst run falls from 295 to 215 ns, while bulk single-producer throughput is unchanged; the fence is paid once per cross-core event, after the bucket copy that already dominates. The latency and the idle-spin floor are fixed at mailbox construction from the `CoreInitializer` and read back with `getLatency()` / `getIdleSpin()` (`src/qb/core/Main.cpp:155`; `src/qb/core/Main.h:664-666`; `src/qb/core/Main.h:664-666`).

A core does not park the moment it finds nothing to do. It keeps polling for an **idle-spin floor** — a span of TIME, `CoreInitializer::setIdleSpin()`, 50 µs by default — measured from the first pass that found nothing, and parks only once that floor has elapsed:
...
_metrics.reset();
```
<!-- src: qb/src/qb/core/VirtualCore.cpp:962-980 -->

A pass is idle when it moved no event — sent, received or I/O — **and** left nothing in the core's own pipe: an actor's tick callback may push to itself, which counts as no activity, and parking over that would delay a local event by up to `latency`. The first idle pass stamps `_idle_since`; a wait that times out keeps the stamp, so an idle core parks again on the next pass rather than re-spinning a whole floor between two timeouts; the clock is read only on idle passes — in **every** latency mode, a latency-0 core included: that ~13 ns `lfence`-ordered read is what paces a spinning core's idle poll of the peer rings (without it the cross-core spin cells measured +25 % — the loop-clock pass of the qb-vs-others audit, `CHANGELOG.md` `[Unreleased]`) — and the park itself only when the core can park at all (`src/qb/core/VirtualCore.h:574-582`; `src/qb/core/VirtualCore.cpp:929-956`). Until 3.0 this was an integer **spin credit** refilled from the previous pass's event count, which parked a one-event-per-pass workload after two or three empty passes — every hop of a request/response paid an OS park and wake (axis A of the qb-vs-others audit). A floor in time is indifferent to how many events a pass carries: a ping-pong whose hops take a few microseconds never reaches it, and a genuinely idle core still yields its CPU after 50 µs. `setIdleSpin(0)` parks on the first idle pass; a larger floor keeps a burst-then-quiet core polling longer (`src/qb/core/Main.h:317`; `src/qb/core/Main.h:368`).
...
- A Windows **GNU** build applies no affinity at all — the code path is `#warning`-ed out (`src/qb/core/VirtualCore.cpp:543-545`).

Do not infer placement from a call that returned. Ask `qb::CPU::ThreadPinningSupported()` (`src/qb/system/cpu.h:189`), which is a runtime probe and is therefore also right for an x86_64 binary under Rosetta 2. The full account is on `qb::NoAffinity` (`src/qb/core/Main.h:76-99`).
...
| Code | Raised when |
|---|---|
| `BadInit` | `VirtualCore::__init__` returned false, or `start()` was called with no core registered at all (`src/qb/core/Main.cpp:523-527`) |
...
The `onInit()`-throws case is worth stating exactly, because it is easy to guess wrong. `__drive_init__` is `noexcept` and catches the exception itself, logs it, and reports `InitOutcome::ReadyFalse` (`src/qb/core/VirtualCore.cpp:602-614`). It therefore lands on `BadActorInit`, identically to a clean `co_return false`. `ExceptionThrown` is reserved for a throw that actually escapes — a throwing constructor during `create()`, or a handler throwing inside `__workflow__`.

Only the boolean is public. `hasError()` reads the barrier counter and compares it against `BadInit`; the individual code is visible only in the log (`src/qb/core/Main.cpp:572-575`). **`join()` returning does not mean success** — always check.

## The loop pass

One iteration of `__workflow__`, in the order it happens (`src/qb/core/VirtualCore.cpp:749-989`):

```mermaid
flowchart TD
    T["1. ++_loop_count — the pass index that keys the lazy time() sample"] --> S
    S["2. signal / stop_token check<br/>synthesise a SignalEvent into this core's own pipe"] --> IO
    IO["3. listener::current.run(EVRUN_NOWAIT)<br/>watchers · deferred · bounded coroutine drain<br/>(gated: listener::has_work)"] --> ACT
    ACT["4. __pump_activations__ — finish async onInit,<br/>replay stashes, enforce deadlines (empty-guarded)"] --> FL
    FL["5. __flush_all__ — scan the peer pipes; drain the non-empty ones into peer mailboxes"] --> RC
    RC["6. __receive__ — walk the same-core pipe in place up to a fence,<br/>then dequeue this core's mailbox, routing each event"] --> K1
    ...
    IDLE["9. idle? stamp _idle_since;<br/>if latency &gt; 0 and the idle-spin floor has elapsed: park INSIDE the io loop<br/>when listener::has_work, else on the mailbox cv"] --> T
```

Six things read off that ordering, and each is a question people actually ask:

- **`Actor::time()` is constant for a whole pass, and sampled on demand.** It forwards to `VirtualCore::time()`, which reads `qb::unix_nanos(qb::wall_now())` on the FIRST call of a pass — keyed on `_loop_count` — and returns that sample to every later call in the same pass (`src/qb/core/VirtualCore.cpp:1228-1235`; `src/qb/core/VirtualCore.h:593-595`). Every actor on the core sees the same value in the same pass, and so does `qb::LoopEvent::now`; a pass in which nobody asks reads no clock at all — through 3.1.0 every pass paid one read at step 1, asked or not, and until the 3.2 loop-clock pass the `LoopEvent` of step 8 still asked on every pass, callbacks or none. For a moving clock read `qb::wall_now()` directly; `Actor::now()` is the same cached instant as a `qb::wall_time`.
- **A `qb::ICallback` tick happens *after* the flush**, so events a tick pushes wait for the *next* pass to leave the core. Events pushed from an ordinary handler in step 6 also miss step 5, for the same reason. One `qb::LoopEvent` is built for the whole pass and handed to every registered callback by a direct virtual call — it is not a routed event and has no source or destination (`src/qb/core/VirtualCore.cpp:852-856`; `src/qb/core/ICallback.h:65-81`) — and it is built only when at least one callback is registered: a core with none skips step 8 entirely (`src/qb/core/VirtualCore.cpp:851`), which is what keeps a callback-free pass clock-free.
...
- **Step 3 is gated on the loop's own liveness.** `listener::has_work()` — a referenced active watcher, a pending event, an outstanding `defer()`, a ready coroutine or a completed spawned frame the scheduler still has to free (an `ask` reply resumes its frame inline, so a coroutine can finish outside a `run()` turn), the first two read inline off the loop's own counters through `ev_active_count_addr()` / `ev_pending_count_addr()` rather than through two calls into libqev per pass (`src/qb/io/async/listener.h:1164-1167`; `src/qb/core/VirtualCore.cpp:811-814`). A pure-actor core with no live qb-io object skips the loop entirely. The gate used to be `has_coro_scheduler() || size() || has_deferred()`, and its first term is true for the rest of a core's life once any actor has spawned a coroutine — a `NOWAIT` libev pass on every tick with nothing to poll for. And a core that owns a socket does not poll it on every pass either: the listener polls the backend while the loop is hot (the previous poll reported a ready fd) and otherwise once per `CoreInitializer::setIoPollInterval` (1 µs by default), the passes in between running `ev_run(EVRUN_NOWAIT | EVRUN_NOPOLL)` — timers and pending events as usual, no syscall — since the poll was the whole of what an io pass cost over a plain one (124 vs 12 ns on WSL2, 275 vs 16 on MSVC) and on a quiet socket it returned nothing on nearly all of them. And a pass with nothing for the loop to do does not run it at all: the listener reads the loop's counts inline — a pending event, a wake left by another thread's `ev_async_send`, a poll the cadence is due to make, a timer within reach of an estimate it keeps on the CPU's counter — and enters `ev_run` only for one of them, handing it the clock reading that found the timer due (`ev_now_set`) so the pass reads the clock once; a busy core with a far timer 36.6 → 26.3 ns per pass, a quiet socket between two polls 48.0 → 27.5, against 12.2 for a plain pass (WSL2 g++-14). The corollary for a raw timer: `event::timer::start()` refreshes the loop's clock before arming, because that clock now stands still between the passes that run the loop.
- **Step 9 parks where a wake can reach it.** Past the idle-spin floor a `latency > 0` core with io work — `listener::has_work()` again — parks INSIDE its io loop: `Mailbox::wait(listener &, cap)` publishes `Park::Loop` and hands the cap — `latency`, or the time to the core's earliest request deadline when a timed `ask` is pending, whichever is smaller (`src/qb/core/VirtualCore.cpp:943-948`) — to `listener::run_once_for` (`src/qb/core/Main.h:553-568`) — one `ev_run(EVRUN_ONCE)` capped by a one-shot timer (`src/qb/io/async/listener.h:1247`) — so a socket that becomes readable ends the park at poll latency, and a producer on another core ends it through the loop's async watcher (`listener::wake()`, `src/qb/io/async/listener.h:1222`) — the one libev entry point safe against a concurrent `ev_run`. Io delivered by that park clears `_idle_since`, so the reply is met at polling latency (`src/qb/core/VirtualCore.cpp:940-954`). A core with no io work keeps the condition-variable park. Before 3.2 every park was the cv, which a readable socket could not end: measured p50 0.9–1.2 ms at `latency` 100 µs and 15.6 ms at 10 ms, against 24–31 µs with the loop park (qb-vs-others, axis N).
- **The loop ends when the last actor is reaped**, not on a flag: `if (_actor_count == 0) break;` (`src/qb/core/VirtualCore.cpp:892-894`). An engine whose actors never call `kill()` runs forever, by design.

### Reaping is a drain, not a sweep

`kill()` only flags. The reap at step 7/8 destroys the flagged actors — and destroying one runs arbitrary user code, which may `kill()` another and re-enter `killActor()`. Iterating the queue being mutated would be a use-after-free (a growth reallocation of the vector moves every entry), so the loop swaps into a scratch buffer and repeats until a pass adds nothing:
...
```
<!-- src: qb/src/qb/core/VirtualCore.cpp:885-890 -->

It terminates because only user code refills the queue and each actor can be removed at most once — after an id leaves `_actors`, `removeActor` destroys nothing. Pinned by `KillDuringReap.ActorKilledFromAnotherDestructorIsStillReaped` (`qb/tests/core/system/lifecycle/kill-during-reap.cpp`). A callback whose actor was killed earlier **in the same pass** is skipped rather than ticked one last time, because the tick loop consults `_actor_to_remove` before every call (`src/qb/core/VirtualCore.cpp:866-870`).
...
A failed `try_send` falls into one of four cases, tested in this order (`src/qb/core/VirtualCore.cpp:340-493`):
...
The backoff is monotonic and its two thresholds are compile-time constants — `kFlushSpinAttempts = 64` and `kFlushYieldAttempts = 512`, `static_assert`ed in that order (`src/qb/core/VirtualCore.cpp:315-317`). After the first `try_send` fails, the retry loop runs a budget of 512 further attempts (`src/qb/core/VirtualCore.cpp:452-464`):
...
- budget exhausted — **partial flush**: `notify()` the destination's mailbox, `pipe.consume_front(cur - seg.data())` so the unsent tail stays queued in order at the head of its segment, and return to the workflow (`src/qb/core/VirtualCore.cpp:473-484`).
...
`Main::start()` calls `Main::install_default_signals()`, which installs handlers for **both** `SIGINT` and `SIGTERM` through `sigaction`, so Ctrl-C and the signal a container runtime or service manager sends both unwind every actor through the ordinary `kill()` path (`src/qb/core/Main.cpp:666-684`). You do not need to register either yourself. `sigaction` rather than `std::signal` is deliberate: under the historical System V semantics `std::signal` resets the disposition after the first delivery, so a second Ctrl-C would terminate the process instead of shutting it down again (`src/qb/core/Main.cpp:623-641`).

The handler itself does the minimum a signal handler may: store the signum, then bump a generation counter with release ordering. Both are `std::atomic` and both are `static_assert`ed lock-free (`src/qb/core/Main.cpp:350-363`).

The generation is what makes the mechanism repeatable. `_signal_pending` holds only the *latest* signum and is never cleared during a run, so a per-core "already consumed" latch would drop every signal after the first — a `SIGHUP` reload followed by a `SIGTERM`, or `Main::stop()` after any earlier signal, would leave the engine unstoppable. Each core re-synthesises whenever the generation advances past the one it last delivered (`src/qb/core/VirtualCore.cpp:769-795`). Signals **coalesce**: two raised between two passes deliver one event carrying the later signum.

`Main::stop()` is the same path with a synthetic `SIGINT`, and is safe from any thread including a signal handler (`src/qb/core/Main.cpp:577-586`). The C++20 `qb::stop_source` is the third: `~Main` requests it, workers observe it through their `stop_token`, and each synthesises a virtual `SIGINT` once — a signal-free shutdown that works identically on platforms with no POSIX signals (`src/qb/core/Main.cpp:370-379`; `src/qb/core/VirtualCore.cpp:770`, `:784-788`).
