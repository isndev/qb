...
Between them sits **`SharedCoreCommunication`**, which `Main` owns and no application code names. It holds the `CoreSet`, one `Mailbox` per used core, and one atomic "has stopped" flag per core (`src/qb/core/Main.h:663-671`). A `Mailbox` is a `qb::lockfree::mpsc::ringbuffer<EventBucket, MaxRingEvents, 0>` (`src/qb/system/lockfree/mpsc.h`) plus the condition variable used by the latency path. The matching send side lives inside each `VirtualCore`: one `qb::VirtualPipe` per destination core, plus one for itself. [Inter-actor messaging](./messaging.md#the-address-is-the-route) narrates a single event through both.
...
```
<!-- src: qb/src/qb/core/Main.h:495-505 -->
...
_metrics.reset();
```
<!-- src: qb/src/qb/core/VirtualCore.cpp:952-970 -->

A pass is idle when it moved no event — sent, received or I/O — **and** left nothing in the core's own pipe: an actor's tick callback may push to itself, which counts as no activity, and parking over that would delay a local event by up to `latency`. The first idle pass stamps `_idle_since`; a wait that times out keeps the stamp, so an idle core parks again on the next pass rather than re-spinning a whole floor between two timeouts; the clock is read only on idle passes — in **every** latency mode, a latency-0 core included: that ~13 ns `lfence`-ordered read is what paces a spinning core's idle poll of the peer rings (without it the cross-core spin cells measured +25 % — the loop-clock pass of the qb-vs-others audit, `CHANGELOG.md` `[Unreleased]`) — and the park itself only when the core can park at all (`src/qb/core/VirtualCore.h:521-529`; `src/qb/core/VirtualCore.cpp:918-946`). Until 3.0 this was an integer **spin credit** refilled from the previous pass's event count, which parked a one-event-per-pass workload after two or three empty passes — every hop of a request/response paid an OS park and wake (axis A of the qb-vs-others audit). A floor in time is indifferent to how many events a pass carries: a ping-pong whose hops take a few microseconds never reaches it, and a genuinely idle core still yields its CPU after 50 µs. `setIdleSpin(0)` parks on the first idle pass; a larger floor keeps a burst-then-quiet core polling longer (`src/qb/core/Main.h:312`; `src/qb/core/Main.h:363`).
...
One iteration of `__workflow__`, in the order it happens (`src/qb/core/VirtualCore.cpp:743-979`):
