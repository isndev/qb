...
| Cancellation-aware context outside `spawn` | `Actor::context()` | Returns the same `ScopedCoroContext` a `spawn()` body receives, available wherever you hold the actor — most notably **inside `onInit()`**, which is what makes a `co_await` during init cancellable when the actor is killed (`src/qb/core/Actor.h:1262`). |
| Resolve a pending `qb::ask` | `Actor::resolve_ask(e)` | Routes a correlated reply arriving in an ordinary `on(E&)` handler back to the awaiting coroutine (`src/qb/core/Actor.h:1298`). |
