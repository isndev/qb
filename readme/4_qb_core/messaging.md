...
```
<!-- src: qb/src/qb/core/VirtualCore.h:1013 (dest._core_id selects the pipe), qb/src/qb/core/VirtualCore.h:1044-1049 (__getPipe__), qb/src/qb/core/Main.cpp:227-232 (the ring enqueue) -->
...
```
<!-- src: qb/src/qb/core/VirtualCore.h:1164-1168 -->

`Actor::send` is the same one-line shape (`src/qb/core/VirtualCore.h:1170-1174`), and so is `Actor::broadcast` (`src/qb/core/VirtualCore.h:1197-1201`). So are the non-template members: `getPipe` (`src/qb/core/Actor.cpp:334-337`), `reply` and `forward` (`src/qb/core/Actor.cpp:424-442`), `time` (`src/qb/core/Actor.cpp:316-319`). No lock, no atomic, no fence appears anywhere on that path.
...
```
<!-- src: qb/src/qb/core/VirtualCore.cpp:1133-1136 -->
...
2. `router::ensure_disposer<Event, E>()` registers a type-erased destructor for `E` if it is not trivially destructible. This runs at the *enqueue* funnel because that is the one place which statically knows the type; without it every drop path later would free bytes without running `~E()` (`src/qb/core/VirtualCore.h:1012`; `src/qb/system/event/router.h:1236-1243`).
...
6. The event is placement-newed into the reservation, then `fill_event` stamps `id`, `dest`, `source` and `bucket_size` into the header (`src/qb/core/VirtualCore.h:1018-1022`, `:953-971`).
...
Nothing filters an event addressed to a dead actor on the way in. The actor stays in the router's handler map until `VirtualCore::removeActor` reaches `unregisterEvents(id)` at the end of the pass (`src/qb/core/VirtualCore.cpp:1068-1069`), so between `kill()` and the reap it is still a routing target. What stops it receiving is the trampoline:
...
`to(dest)` returns an `Actor::EventBuilder`, which holds a `qb::Pipe` and whose `push` forwards to `Pipe::push` and returns the builder for chaining (`src/qb/core/Actor.h:634-672`; `src/qb/core/VirtualCore.h:1220-1225`):
...
```
<!-- src: qb/src/qb/core/VirtualCore.h:1018 -->
...
```
<!-- src: qb/src/qb/core/VirtualCore.h:975-993 -->
...
```
<!-- src: qb/src/qb/core/VirtualCore.h:961-963 -->
...
|---|---|---|
| `reply` / `forward` | both route through `VirtualCore::send(Event const&)`: same core, or a cross-core `try_send` miss, it is `detail::event_wire::copy` into a fresh pipe reservation — one 16-byte header move plus the tail (`src/qb/core/VirtualCore.cpp:1155-1160`; `src/qb/core/Event.h:693-723`); a cross-core hit is the raw mailbox copy of step 10 | **yes** |
...
```
<!-- src: qb/src/qb/core/VirtualCore.cpp:1169-1174 -->

`forward` is the same three lines with `detail::event_wire::set_dest(event, dest)` in place of the swap, and **deliberately leaves `event.source` untouched** so a downstream `reply` returns to the true client rather than to the forwarder (`src/qb/core/VirtualCore.cpp:1176-1181`). In one line: *`reply` swaps `dest` and `source`; `forward` sets a new `dest` and keeps `source`.*
...
- **`send(Event const&)` refuses to relocate a raised flag.** Its cross-core fast path is the raw mailbox copy of the original bytes, which would carry the bit; an already-replied event handed back to it (`Actor::send(Event const&)` is module API) goes through the local pipe instead, whose copy clears it (`src/qb/core/VirtualCore.cpp:1155-1160`). Before 3.2 the receive-side store masked exactly this case, and the shape it hid — forward cross-core after a reply — delivered an event the receiver never destroyed.
