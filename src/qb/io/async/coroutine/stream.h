...
public:
     ...
     */
    static async_stream
    from_channel(channel<T> &ch) {
        channel<T> *ch_ptr = &ch;
        return async_stream([ch_ptr]() -> task<std::optional<T>> { co_return co_await ch_ptr->recv(); });
    }
     ...
     */
    static async_stream
    from_channel_shared(std::shared_ptr<channel<T>> ch) {
        return async_stream([ch]() -> task<std::optional<T>> { co_return co_await ch->recv(); });
    }
     ...
     */
    static async_stream
    from_vector(const std::vector<T> &vec) {
    ...
    static async_stream
    empty() {
        return async_stream([]() -> task<std::optional<T>> { co_return std::nullopt; });
    }
    ...
    template <typename F>
    auto
    map(F f) -> async_stream<std::invoke_result_t<F, T>> {
    ...
    debounce(qb::duration delay) {
        ...
        return async_stream([source, ch, started, delay, source_error]() -> task<std::optional<T>> {
            ...
            while (true) {
                ...
                if (got_new)
                    continue; // new values arrived — restart quiet period
     ...
     */
    async_stream
    throttle(qb::duration interval) {
        auto source    = _next;
        auto last_emit = std::make_shared<std::chrono::steady_clock::time_point>(std::chrono::steady_clock::now() - interval);
    ...
    template <typename F>
    static task<void>
    for_each_impl(async_stream<T> stream, F f) {
        while (true) {
    ...
    static task<std::vector<T>>
    collect_impl(async_stream<T> stream) {
        std::vector<T> result;
        while (true) {
            auto opt = co_await stream._next();
            if (!opt)
    ...
    static task<std::optional<T>>
    first_impl(async_stream<T> stream) {
    ...
    static task<size_t>
    count_impl(async_stream<T> stream) {
        size_t n = 0;
        while (true) {
            auto opt = co_await stream._next();
            if (!opt)
    ...
    template <typename P>
    static task<bool>
    any_impl(async_stream<T> stream, P predicate) {
        while (true) {
            auto opt = co_await stream._next();
            if (!opt)
                co_return false;
    ...
    template <typename P>
    static task<bool>
    all_impl(async_stream<T> stream, P predicate) {
        while (true) {
            auto opt = co_await stream._next();
            if (!opt)
                co_return true;
    ...
    template <typename P>
    static task<std::optional<T>>
    find_impl(async_stream<T> stream, P predicate) {
        while (true) {
            auto opt = co_await stream._next();
            if (!opt)
                co_return std::nullopt;
    ...
    static task<void>
    drain_to_impl(async_stream<T> stream, channel<T> *ch_ptr) {
        while (true) {
            auto opt = co_await stream._next();
            if (!opt)
                co_return;
    ...
    std::function<task<std::optional<T>>()> &
    next_fn() {
        return _next;
    }
};
...
template <typename T>
async_stream<T>
merge_streams(std::vector<async_stream<T>> streams) {
    if (streams.empty()) {
...
async_stream<std::pair<T, U>>
zip(async_stream<T> a, async_stream<U> b) {
...
template <typename T>
async_stream<T>
repeat_value(T value) {
    auto shared = std::make_shared<T>(std::move(value));
    return async_stream<T>([shared]() -> task<std::optional<T>> { co_return *shared; });
}
 ...
 * @ingroup Coroutine
 */
inline async_stream<size_t>
interval(qb::duration interval_time, bool start_with_now = false) {
    auto counter   = std::make_shared<size_t>(0);
    auto next_time = std::make_shared<std::chrono::steady_clock::time_point>(std::chrono::steady_clock::now()
                                                                             + (start_with_now ? qb::duration::zero() : interval_time));
...
template <typename T>
async_stream<T>
timer(T value, qb::duration delay) {
    auto shared  = std::make_shared<std::optional<T>>(std::move(value));
