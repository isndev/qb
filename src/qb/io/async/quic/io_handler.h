...
struct stream_key {
    ...
    [[nodiscard]] friend bool operator==(stream_key const &, stream_key const &) noexcept = default;
};
...
public:
    ...
    using session_map_t = qb::unordered_map<stream_key, std::shared_ptr<StreamSession>, stream_key_hash>;
...
public:
    ...
    session(std::uint64_t stream_id) noexcept {
    ...
    [[nodiscard]] StreamSession *
    session(std::uint64_t connection_id, std::uint64_t stream_id) noexcept {
        auto it = _sessions.find({connection_id, stream_id});
        return it == _sessions.end() ? nullptr : it->second.get();
    }

    [[nodiscard]] StreamSession const *
    session(std::uint64_t stream_id) const noexcept {
    ...
    [[nodiscard]] StreamSession const *
    session(std::uint64_t connection_id, std::uint64_t stream_id) const noexcept {
        auto it = _sessions.find({connection_id, stream_id});
        return it == _sessions.end() ? nullptr : it->second.get();
    }

    [[nodiscard]] std::shared_ptr<StreamSession>
    session_handle(std::uint64_t stream_id) noexcept {
    ...
    [[nodiscard]] std::shared_ptr<StreamSession>
    session_handle(std::uint64_t connection_id, std::uint64_t stream_id) noexcept {
        auto it = _sessions.find({connection_id, stream_id});
        return it == _sessions.end() ? nullptr : it->second;
    }

    [[nodiscard]] std::shared_ptr<const StreamSession>
    session_handle(std::uint64_t stream_id) const noexcept {
        if (auto exact = session_handle(0, stream_id))
            return exact;
    ...
    [[nodiscard]] std::shared_ptr<const StreamSession>
    session_handle(std::uint64_t connection_id, std::uint64_t stream_id) const noexcept {
        auto it = _sessions.find({connection_id, stream_id});
        return it == _sessions.end() ? nullptr : it->second;
    }

    [[nodiscard]] StreamSession *
    registerSession(std::uint64_t stream_id) {
        return registerSession(0, stream_id);
    }

    [[nodiscard]] StreamSession *
    registerSession(std::uint64_t connection_id, std::uint64_t stream_id) {
    ...
    [[nodiscard]] StreamSession *
    register_stream_session(std::uint64_t stream_id) {
        return registerSession(stream_id);
    }

    [[nodiscard]] StreamSession *
    register_stream_session(std::uint64_t connection_id, std::uint64_t stream_id) {
        return registerSession(connection_id, stream_id);
    }

    void
    unregisterSession(std::uint64_t stream_id) {
        unregisterSession(0, stream_id);
    }

    void
    unregisterSession(std::uint64_t connection_id, std::uint64_t stream_id) {
        _sessions.erase({connection_id, stream_id});
    }

    void
    disconnected(std::uint64_t stream_id) {
        unregisterSession(stream_id);
    }

    void
    disconnected(std::uint64_t connection_id, std::uint64_t stream_id) {
        unregisterSession(connection_id, stream_id);
    }

    void
    clearSessions() noexcept {
        _sessions.clear();
    }

    void
    clearSessions(std::uint64_t connection_id) noexcept {
...
protected:
    [[nodiscard]] StreamSession *
    ensure_stream_session(std::uint64_t stream_id) {
        return registerSession(0, stream_id);
    }

    [[nodiscard]] StreamSession *
    ensure_stream_session(std::uint64_t connection_id, std::uint64_t stream_id) {
        return registerSession(connection_id, stream_id);
    }

    bool
    feed_stream_data(event::stream_data const &ev) {
        return feed_stream_data(ev, [](std::uint64_t, std::uint64_t, std::uint64_t) {});
    }

    template <typename Credit>
    bool
    feed_stream_data(event::stream_data const &ev, Credit &&credit) {
        ...
        if constexpr (requires(StreamSession &s, std::string_view data) {
                          s.append(data);
                          s.process();
                      }) {
    ...
    void
    drain_stream_output(StreamSession &session, Send &&send) {
        if constexpr (requires(StreamSession &s) {
                          s.pendingWrite();
                          s.out();
                          s.id();
                      }) {
            while (session.pendingWrite() > 0) {
                ...
                send(session.connection_id(), session.id(), std::span<const std::byte>(reinterpret_cast<const std::byte *>(out.begin()), size),
                     false);
...
private:
    static void
    assign_stream_id(StreamSession &session, std::uint64_t stream_id) {
        if constexpr (requires(StreamSession &s) { s.assign_stream_id(stream_id); })
            session.assign_stream_id(stream_id);
    }

    static void
    assign_quic_ids(StreamSession &session, std::uint64_t connection_id, std::uint64_t stream_id) {
