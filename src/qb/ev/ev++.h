...
struct loop_ref {
    ...
    unsigned int
    pending_count() const EV_NOEXCEPT {
        return ev_pending_count(EV_AX);
    }

    unsigned int
    active_count() const EV_NOEXCEPT {
    ...
    void
    once(int fd, int events, tstamp timeout, K *object) EV_NOEXCEPT {
    ...
    void
    once(int fd, int events, tstamp timeout) EV_NOEXCEPT {
...
void
start(int fd, int events) EV_NOEXCEPT {
    set(fd, events);
    start();
}
...
void
start(int pid, int trace = 0) EV_NOEXCEPT {
    set(pid, trace);
    start();
}
